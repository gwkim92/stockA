...
Latest evidence/visibility update: `internal-rag-retrieval-foundation-v1`, `ai-evidence-visibility-v3`, `cycle-quality-audit-hardening-v1`, `decision-cockpit-recommendation-boundary-summary-v1`, and `professional-analysis-quality-v1` are implemented and EC2-smoked. Latest commits are `084010b`/`bfa11d2` for internal RAG, `3b4c2c9`/`17deba7` for AI evidence visibility, `ced0d9d`/`d75d5e2` for cycle quality audit, `56e1e04`/`fa54517` for recommendation boundary summary, and `4f79661`/`87244ae` for professional analysis quality. `/api/data-health` exposes `professional_analysis_quality.status=managed_source_limited`, active candidates `23`, complete candidates `22`, source blocked `1`, average coverage `0.9674`, `automatic_weight_change_allowed=false`, `broker_submit_allowed=false`, `order_boundary=read_only_no_order`, and `open_gates=[]`. `/data-health` renders `전문 분석 품질`, `재무·피어·밸류에이션·산업·AI 리서치`, `원천 한계 관리 중`, `weight 변경 금지`, and `읽기 전용·주문 금지`.
...
1. `professional-recommendation-coverage-audit-v1`: audit each active recommendation for attached financial metrics, peer comparison, valuation, industry positioning, AI research artifact, thesis, paper validation, and source blocker state.
2. `recommendation-detail-professional-evidence-v2`: make recommendation detail read like a professional evidence waterfall, separating news/cycle evidence from financial/valuation/peer/industry evidence and showing blockers inline.
3. `outcome-maturity-wait-monitor-v2`: keep the 2026-06-20 and 2026-06-24 outcome windows explicit and route follow-up calibration only when evidence is due.
4. `manual-weight-review-pilot-v1`: only after mature outcome evidence exists, open a separate approved pilot task to consider small manual weight changes. Until then all recommendation weights remain unchanged.
