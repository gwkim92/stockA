...
Latest operational gate update: `auth-rbac-readonly-boundary-v1` is implemented locally and pending EC2 smoke. It adds `auth_rbac` readiness evidence to `/api/data-health`, maps production read-token access to a read-only role, keeps write methods blocked with 405, and keeps broker/order submit blocked by `read_only_no_order`. The previous `alert-destination-readiness-visibility-v1` evidence remains active: `/api/data-health` exposes `alert_destination.status=missing_destination`, `attention_required=true`, `mode=missing`, `target_configured=false`, and `last_test_status=missing`; the `alert_destination` open gate intentionally remains open until an external destination and recent passed test artifact exist. The previous `production-api-server-gate-evidence-v1` evidence remains active with `production_api_server.status=production_ready`, `attention_required=false`, `runtime_profile=production`, `source_mode=live`, `auth_mode=read-token`, and `connection_boundary=psycopg_pool`, so the static `production_api_server` gate stays closed. Expected remaining open gates after EC2 auth/RBAC smoke are `alert_destination` and `portfolio_review_feedback_calibration_attention`.
