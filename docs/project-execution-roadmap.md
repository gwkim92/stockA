...
Latest status update: `professional-source-gap-managed-gate-v1` is implemented and EC2-smoked on commit `657e95d`. `/api/data-health` still exposes EROK as a source limitation, but `professional_source_gap_prioritization.attention_required=false` because EROK is already blocked from professional decision use and paper validation; open gates dropped from 8 to 7 and no longer include `professional_source_gap_attention`. `/data-health` renders `원천 한계 관리 중` and `전문 판단과 페이퍼 검증 입력에서는 이미 차단`. Recommendation outcome due action remains waiting until `2026-06-20`; recommendation weights and broker submit remain blocked.
