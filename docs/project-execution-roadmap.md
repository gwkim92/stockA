...
1. `portfolio-risk-budget-frontend-guardrail-visibility`: expose the persisted guardrail run/eval result on paper trading and portfolio safety screens.
2. `portfolio-risk-budget-benchmark-composition-v1`: add a free benchmark composition source or explicit manual composition seed before drift can be calculated.
...
19. `portfolio-risk-budget-frontend-guardrail-visibility`: next task; make the persisted guardrail and paper validation risk blockers visible to users.
