...
Latest operational gate update: `data-operations-artifact-runner-gate-evidence-v1` is implemented and EC2-smoked on commit `2bdb739`. `/api/data-health` now exposes `data_operations_artifact_runner.status=operational_profile_scheduler_active`, `artifact_policy_count=33`, `job_count=33`, `latest_run_count=33`, and `active_timer_count=7/timer_count=7`. The static `data_operations_artifact_runner` open gate is removed when this evidence is present. Remaining open gates are `production_api_server`, `auth_rbac`, `alert_destination`, and `portfolio_review_feedback_calibration_attention`.
