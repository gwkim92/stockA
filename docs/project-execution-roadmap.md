...
66. `portfolio-review-decision-history-v1`: implemented locally; portfolio review decisions are persisted as read-only `ai.eval_run` history, exposed on data-health and portfolio coverage, and keep recommendation scoring, benchmark definition, portfolio positions, and broker submit unchanged.
67. `portfolio-review-decision-outcome-feedback-v1`: planned as the next task; saved portfolio review decisions should be checked against later paper validation, recommendation outcome, thesis, and price evidence before any future weight pilot.
