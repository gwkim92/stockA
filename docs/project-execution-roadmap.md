...
| Professional Analysis | Financial normalization, financial statement model detail, peer relative snapshots, financial forecast inputs, sum-of-the-parts valuation foundation, segment/footnote evidence, reported segment parsing, valuation target range, valuation quality depth, portfolio risk budget, position sizing, professional decision waterfall, thesis gates, segment history backfill, segment history coverage expansion, breadth expansion, `aeis-reported-segment-parser-layout-v1`, `segment-history-source-linkage-remediation-v1`, `professional-coverage-refresh-after-source-remediation-v1`, `portfolio-and-fund-instrument-analysis-v1`, `fund-expense-tracking-source-v1`, `fund-expense-ratio-public-source-v1`, `fund-nav-premium-discount-source-v1`, `fund-tracking-error-source-v1`, `recommendation-outcome-calibration-sample-expansion-v1`, `recommendation-weight-review-horizon-gate-v1`, `recommendation-outcome-maturity-monitor-v1`, `recommendation-outcome-due-cadence-automation-v1`, `professional-source-gap-prioritization-v1`, `professional-source-gap-remediation-decision-v1`, and `professional-source-blocker-raw-filing-remediation-v1` exist. Latest raw filing blocker evidence is EC2 commit `9ff096f`: runner `run_id=1607`, `eval_run_id=30`, `EROK` decision `durable_exclusion_until_periodic_filing`, blocker `ipo_prospectus_without_standard_periodic_financials`, companyfacts namespace `ffd` only, supported periodic filing count `0`, latest prospectus `424B4` filed `2026-05-14`; `/api/data-health` exposes `raw_filing_decision.eval_run_id=eval-run-30`, `automatic_order_allowed=false`, `broker_submit_allowed=false`; `/`, `/data-health`, and `/stocks/EROK` route smoke returned `200`. | Automatic weight changes remain blocked. Live broker submit remains excluded. Current next work remains `cycle-ai-quality-audit-contamination-remediation-v1`, because quality audit still reports one stale `SPY` direct impact and one duplicate title issue. |

Latest status update: `cycle-ai-quality-audit-contamination-remediation-v1` first remediation is deployed on EC2 commit `9ca5905`. Root cause for the largest issue class was an audit false-positive: the quality audit did not normalize company-name tokens and did not recognize curated ETF/index aliases already used by the news validator/enrichment policy. EC2 audit rerun `run_id=1619` now reports `issue_count=3`, `readiness_gap_count=0`, `ungrounded_direct_ticker_count=1`, `macro_false_ticker_count=1`, `duplicate_title_count=1`, and `quantum_energy_mislink_count=0`. Next work remains stale-impact cleanup for `event_id=19`/`SPY` and duplicate title remediation.
...
1. `cycle-ai-quality-audit-contamination-remediation-v1`: continue after the first grounding remediation by removing stale direct impacts whose current source text no longer grounds the ticker, starting with `event_id=19`/`SPY`; then remediate duplicate RSS title clustering.
...
63. `cycle-ai-quality-audit-contamination-remediation-v1`: first grounding remediation implemented and EC2-smoked on commit `9ca5905`; issue count dropped from `12` to `3`, but the task remains open for stale impact and duplicate title cleanup.
