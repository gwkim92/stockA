...
1. `financial-period-source-document-linkage-v1`: improve SEC filing/source-document linkage and raw artifact availability so segment parser candidates are not blocked by missing `market.financial_statement_period.source_document_id`.
...
36. `financial-period-source-document-linkage-v1`: next task; improve SEC filing/source-document linkage and raw artifact coverage so the parser has real candidates on EC2.
