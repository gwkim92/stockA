...
1. `news-intraday-scheduler-failure-remediation-v1`: diagnose the EC2 `news-intraday` service `exit-code` result, fix the backend profile or runtime env issue, rerun the service, and confirm data-health shows the profile healthy.
2. `cycle-ai-quality-audit-contamination-remediation-v1`: reduce ungrounded direct tickers, macro false ticker counts, and duplicate RSS titles before trusting AI evidence as recommendation input.
3. After `recommendation_outcome_maturity.next_due_date=2026-06-20`, rerun outcome calibration and keep recommendation weight review blocked until due outcome data exists.
4. Keep EROK excluded from company-financial recommendation coverage until the first supported periodic filing appears or a dedicated prospectus/pro-forma parser is implemented.
