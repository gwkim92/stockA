...
| Professional Analysis | Financial normalization, financial statement model detail, peer relative snapshots, financial forecast inputs, sum-of-the-parts valuation foundation, segment/footnote evidence, reported segment parsing, valuation target range, valuation quality depth, portfolio risk budget, position sizing, professional decision waterfall, thesis gates, segment history backfill, segment history coverage expansion, breadth expansion, `aeis-reported-segment-parser-layout-v1`, `segment-history-source-linkage-remediation-v1`, `professional-coverage-refresh-after-source-remediation-v1`, and `portfolio-and-fund-instrument-analysis-v1` exist. Latest fund analysis evidence is EC2 commit `ea9a0dc`: `/api/stocks/SPY` and `/api/recommendations/recommendation-157` expose `fund_instrument_analysis.status=available`, `benchmark_source=ssga_spdr_spy_daily_holdings`, `holding_count=503`, `holdings_coverage_weight=0.9983782`, top holdings `NVDA/AAPL/MSFT/AMZN/GOOGL`, and `order_boundary=read_only_no_order`. Source blockers are still user-facing: SPY/fund-like products show `fund_company_financial_model_not_applicable`, EROK shows `sec_companyfacts_missing_us_gaap_facts`, and ARM remains available without polluted segment labels. | Automatic weight changes remain blocked. Live broker submit remains excluded. Current next work is `fund-expense-tracking-source-v1`: add free/explicit-source handling for ETF expense ratio, tracking error/NAV drift, and liquidity evidence. |

Latest status update: `portfolio-and-fund-instrument-analysis-v1` is complete. EC2 commit `ea9a0dc` adds `fund_instrument_analysis` to stock and recommendation details. SPY now renders as a fund-like instrument with SSGA holdings source, 503 holdings, coverage `0.9983782`, top holdings `NVDA/AAPL/MSFT/AMZN/GOOGL`, explicit tracking-error/expense-ratio unknown states, and read-only order boundary. This does not change recommendation weights or broker/order flow.
...
1. `fund-expense-tracking-source-v1`: add free/explicit-source handling for ETF expense ratio, tracking error/NAV drift, and liquidity evidence so fund analysis no longer leaves those fields at placeholder unknown when public data is available.
...
51. `fund-expense-tracking-source-v1`: next task; add free/explicit-source handling for ETF expense ratio, tracking error/NAV drift, and liquidity evidence while preserving unknown states where source data is absent.
