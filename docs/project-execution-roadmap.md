...
1. `portfolio-risk-budget-benchmark-composition-v1`: add a free benchmark composition source or explicit manual composition seed before drift can be calculated.
2. `portfolio-risk-budget-drift-frontend-visibility`: after benchmark composition exists, show benchmark drift as a measured value instead of an explicit unavailable state.
...
20. `portfolio-risk-budget-benchmark-composition-v1`: next task; add free/manual benchmark composition so drift can be calculated instead of marked unavailable.
