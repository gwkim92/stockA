...
1. `segment-history-coverage-breadth-expansion-v1`: run broader active recommendation/portfolio segment coverage and prioritize remaining non-single-segment parser/data blockers.
...
46. `segment-history-coverage-breadth-expansion-v1`: next task; expand the coverage run beyond AAPL/ADI to the active recommendation and portfolio symbol set, then rank remaining parser/data blockers for deterministic remediation.
