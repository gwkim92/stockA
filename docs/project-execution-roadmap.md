...
- `financial-forecast-and-scenario-inputs-v1`: implemented locally; explicit revenue, margin, CAPEX, and FCF forecast input rows now feed valuation assumptions and frontend forecast evidence without score or order changes.

Current remaining task:

1. `sum-of-the-parts-valuation-foundation-v1`: add conservative SOTP valuation evidence for multi-segment businesses before any recommendation weight changes.
...
32. `financial-forecast-and-scenario-inputs-v1`: implemented locally; static DCF-lite/scenario assumptions now have explicit forecast inputs for revenue, margin, CAPEX, and FCF without changing recommendation weights.
33. `sum-of-the-parts-valuation-foundation-v1`: next task; add a conservative SOTP valuation evidence layer for businesses where segment/thematic exposure data is available, while keeping score weights unchanged.
