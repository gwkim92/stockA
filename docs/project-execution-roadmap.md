...
| Professional Analysis | Financial normalization, financial statement model detail, peer relative snapshots, financial forecast inputs, sum-of-the-parts valuation foundation, segment/footnote evidence, reported segment parsing, valuation target range, valuation quality depth, portfolio risk budget, position sizing, professional decision waterfall, thesis gates, segment history backfill, segment history coverage expansion, breadth expansion, `aeis-reported-segment-parser-layout-v1`, `segment-history-source-linkage-remediation-v1`, `professional-coverage-refresh-after-source-remediation-v1`, `portfolio-and-fund-instrument-analysis-v1`, `fund-expense-tracking-source-v1`, `fund-expense-ratio-public-source-v1`, `fund-nav-premium-discount-source-v1`, `fund-tracking-error-source-v1`, and `recommendation-outcome-calibration-sample-expansion-v1` exist. Latest outcome calibration evidence is EC2 commit `8c3cbb1`: `recommendation-outcome-calibration-sample-expansion-run --execute` produced `run_id=1595`, `eval_run_id=27`, nested backfill `run_id=1596`, nested quality eval `eval_run_id=26`, status `no_due_outcome_window`, recommendation-horizon count `180`, recommendation count `45`, outcome count `0`, not-due count `180`, component diagnostic count `10`, `recommendation_scoring_mutated=false`, `automatic_order_allowed=false`, `broker_submit_allowed=false`, and `order_boundary=read_only_no_order`. Source blockers are still user-facing: SPY/fund-like products show `fund_company_financial_model_not_applicable`, EROK shows `sec_companyfacts_missing_us_gaap_facts`, and ARM remains available without polluted segment labels. | Automatic weight changes remain blocked. Live broker submit remains excluded. Current next work is `recommendation-weight-review-horizon-gate-v1`: make manual weight review consume the new horizon-grid outcome gate before any pilot review. |

Latest status update: `recommendation-outcome-calibration-sample-expansion-v1` is complete. EC2 commit `8c3cbb1` adds a horizon-grid outcome calibration runner and `/data-health` visibility. The latest execute result is `run_id=1595`, `eval_run_id=27`, status `no_due_outcome_window`: current active recommendations have 180 tracked recommendation-horizon rows across 30/90/180/365 days, but all are not due yet. This does not change recommendation weights or broker/order flow.
...
1. `recommendation-weight-review-horizon-gate-v1`: make manual weight review readiness consume the latest horizon-grid outcome calibration eval before accepting any `ready_for_weight_review` quality eval.
...
56. `recommendation-weight-review-horizon-gate-v1`: next task; make manual weight review consume the horizon-grid outcome calibration gate before any pilot review.
