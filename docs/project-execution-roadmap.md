...
| Professional Analysis | Financial normalization, financial statement model detail, peer relative snapshots, financial forecast inputs, sum-of-the-parts valuation foundation, segment/footnote evidence, reported segment parsing, valuation target range, valuation quality depth, portfolio risk budget, position sizing, professional decision waterfall, thesis gates, segment history backfill, segment history coverage expansion, breadth expansion, `aeis-reported-segment-parser-layout-v1`, `segment-history-source-linkage-remediation-v1`, `professional-coverage-refresh-after-source-remediation-v1`, ETF/fund evidence, source blocker guardrails, portfolio review feedback, attribution, professional analysis visibility, recommendation-level coverage audit, recommendation detail professional evidence audit, and outcome maturity wait monitor exist. Latest outcome wait evidence is commit `88a233d`: `/api/data-health.outcome_maturity_wait_monitor.status=managed_wait`, recommendation next due `2026-06-20`, next due count `19`, portfolio feedback maturity `2026-06-24`, mature decision gap `10`, `weight_review_blocked=true`, `manual_weight_review_allowed=false`, `automatic_weight_change_allowed=false`, `broker_submit_allowed=false`, `order_boundary=read_only_no_order`, and `/data-health` renders `성과 성숙 대기 모니터`. Latest recommendation detail evidence is commit `0fefb50`: `/api/recommendations/recommendation-67.professional_evidence_audit.status=source_blocked`, coverage `0.2778`, missing layers `6`, `paper_validation_status=blocked_source`, `paper_validation_input_allowed=false`, `source_blocker.blocker_code=sec_companyfacts_missing_us_gaap_facts`, `paper_validation` layer `blocked`, and `/recommendations/recommendation-67` renders `추천 전문 분석 감사`, `전문 원천 차단 추천`, `근거 커버리지`, `거래 경계`, and `weight 변경`. Latest EC2 monthly attribution evidence remains commit `4809684`, direct attribution runner `run_id=1704`, and `performance-monthly.failed_step_count=0`. | Automatic weight changes remain blocked. Live broker submit remains excluded. Outcome windows remain immature; the next permitted action is waiting until the 2026-06-20 recommendation outcome window and 2026-06-24 portfolio feedback maturity date. |
...
1. Wait until the existing 2026-06-20 recommendation outcome window and 2026-06-24 portfolio feedback maturity date, then let the existing due action router decide whether calibration is due.
2. `manual-weight-review-pilot-v1`: only after mature outcome evidence exists, open a separate approved pilot task to consider small manual weight changes. Until then all recommendation weights remain unchanged.
