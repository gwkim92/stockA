...
| Professional Analysis | Financial normalization, financial statement model detail, peer relative snapshots, financial forecast inputs, sum-of-the-parts valuation foundation, segment/footnote evidence, reported segment parsing, valuation target range, valuation quality depth, portfolio risk budget, position sizing, professional decision waterfall, thesis gates, segment history backfill, segment history coverage expansion, breadth expansion, `aeis-reported-segment-parser-layout-v1`, and `segment-history-source-linkage-remediation-v1` exist. Latest source-linkage remediation EC2 smoke `run_id=1452` reports `trend_backed=4`, `single_reportable_segment_no_disaggregated_segment_table=5`, `sec_companyfacts_missing_us_gaap_facts=1`, `unsupported_layout_count=0`, `recommendation_scoring_mutated=false`, `automatic_order_allowed=false`, `broker_submit_allowed=false`, `order_boundary=read_only_no_order`. Latest EC2 quality eval reached `ready_for_weight_review` with professional coverage `30/36 = 0.833333` and outcome count `30`. | Automatic weight changes remain blocked. Live broker submit remains excluded. Current next work is `professional-coverage-refresh-after-source-remediation-v1`: refresh professional coverage and user-facing stock/recommendation evidence after source truth cleanup. |

Latest status update: `segment-history-source-linkage-remediation-v1` is complete. ARM is remediated through `20-F` annual companyfacts support and parser cleanup; EC2 coverage run `1416` classifies ARM as `single_reportable_segment_no_disaggregated_segment_table` with `arm_reported_segment_metric_count=0` and `unsupported_layout_count=0`. EROK is now a precise source blocker; EC2 breadth run `1452` classifies it as `sec_companyfacts_missing_us_gaap_facts` because SEC companyfacts currently exposes no `facts.us-gaap` financial statement taxonomy.
...
1. `professional-coverage-refresh-after-source-remediation-v1`: rerun professional coverage and downstream stock/recommendation evidence checks after source truth cleanup, proving ARM polluted labels remain absent and EROK appears as source-data unavailable rather than parser failure.
...
49. `professional-coverage-refresh-after-source-remediation-v1`: next task; refresh professional coverage and user-facing stock/recommendation evidence after source truth cleanup without changing recommendation weights or order boundaries.
