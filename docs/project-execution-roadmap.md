...
1. `segment-history-backfill-v1`: backfill historical reported segment periods from prior SEC filings so segment assumption calibration can use trend-backed evidence instead of single-period proxy-backed evidence where possible.
...
43. `segment-history-backfill-v1`: next task; backfill historical reported segment periods from prior SEC filings so segment assumption calibration becomes trend-backed instead of single-period proxy-backed where possible.
