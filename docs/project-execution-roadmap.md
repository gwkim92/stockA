...
Latest status update: `data-health-attention-classification-v1` is implemented and EC2-smoked on commit `6dbeeec`. `/api/data-health` still preserves `open_gates`, but now also returns `open_gate_details` with `open_gate_count=8` and `open_gate_detail_count=8`; benchmark drift is classified as `investment_review`, feedback calibration as `outcome_wait`, and professional source gaps as `source_limit`. `/data-health` renders Korean cards including `벤치마크 괴리 검토`, `전문 분석 원천 한계`, `성과 관찰 대기`, `포트폴리오 검토 결정`, and `성숙한 검토 표본 0/10개`. Recommendation outcome due action remains waiting until `2026-06-20`; recommendation weights and broker submit remain blocked.
