...
Latest status update: `segment-history-coverage-breadth-expansion-v1` is now complete and EC2-smoked on commit `856296d`; the authoritative next task is `aeis-reported-segment-parser-layout-v1`.
...
1. `aeis-reported-segment-parser-layout-v1`: inspect AEIS raw SEC artifacts and add deterministic parser support or a more specific documented blocker for the remaining true unsupported layout.
...
47. `aeis-reported-segment-parser-layout-v1`: next task; inspect AEIS raw SEC artifacts and add deterministic parser support or a precise unsupported reason without changing recommendation weights or order boundaries.
