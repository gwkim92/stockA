...
| Professional Analysis | Financial normalization, financial statement model detail, peer relative snapshots, financial forecast inputs, sum-of-the-parts valuation foundation, segment/footnote evidence, reported segment parsing, valuation target range, valuation quality depth, portfolio risk budget, position sizing, professional decision waterfall, thesis gates, segment history backfill, segment history coverage expansion, breadth expansion, `aeis-reported-segment-parser-layout-v1`, `segment-history-source-linkage-remediation-v1`, `professional-coverage-refresh-after-source-remediation-v1`, ETF/fund evidence, source blocker guardrails, portfolio review feedback, attribution, professional analysis visibility, recommendation-level coverage audit, and recommendation detail professional evidence audit exist. Latest recommendation detail evidence is commit `0fefb50`: `/api/recommendations/recommendation-67.professional_evidence_audit.status=source_blocked`, coverage `0.2778`, missing layers `6`, `paper_validation_status=blocked_source`, `paper_validation_input_allowed=false`, `source_blocker.blocker_code=sec_companyfacts_missing_us_gaap_facts`, `paper_validation` layer `blocked`, and `/recommendations/recommendation-67` renders `추천 전문 분석 감사`, `전문 원천 차단 추천`, `근거 커버리지`, `거래 경계`, and `weight 변경`. Latest professional quality evidence is commit `4f79661`: `/api/data-health.professional_analysis_quality.status=managed_source_limited`, active candidates `23`, complete candidates `22`, source blocked `1`, average coverage `0.9674`, `automatic_weight_change_allowed=false`, `broker_submit_allowed=false`, `order_boundary=read_only_no_order`, and `/data-health` renders `전문 분석 품질`. Latest recommendation-level audit evidence is commit `837a5b2`: `/api/data-health.professional_recommendation_coverage_audit.status=source_limited`, recommendation rows `45`, source blocked `1`, paper validation pending `44`, average coverage `0.9833`, and first row `recommendation-67 EROK blocked_source blocked_source read_only_no_order`. Latest EC2 monthly attribution evidence remains commit `4809684`, direct attribution runner `run_id=1704`, and `performance-monthly.failed_step_count=0`. | Automatic weight changes remain blocked. Live broker submit remains excluded. Outcome windows remain immature, so weight review is still blocked until later evidence accumulates. Next work is outcome maturity wait monitoring. |
...
Latest evidence/visibility update: `internal-rag-retrieval-foundation-v1`, `ai-evidence-visibility-v3`, `cycle-quality-audit-hardening-v1`, `decision-cockpit-recommendation-boundary-summary-v1`, `professional-analysis-quality-v1`, `professional-recommendation-coverage-audit-v1`, and `recommendation-detail-professional-evidence-v2` are implemented and EC2-smoked. Latest commits are `084010b`/`bfa11d2` for internal RAG, `3b4c2c9`/`17deba7` for AI evidence visibility, `ced0d9d`/`d75d5e2` for cycle quality audit, `56e1e04`/`fa54517` for recommendation boundary summary, `4f79661`/`87244ae` for professional analysis quality, `837a5b2` for recommendation-level professional coverage audit, and `caca2d9`/`0fefb50` for recommendation detail professional evidence audit. `/api/data-health` exposes `professional_analysis_quality.status=managed_source_limited`, active candidates `23`, complete candidates `22`, source blocked `1`, average coverage `0.9674`, and `professional_recommendation_coverage_audit.status=source_limited`, recommendation rows `45`, source blocked `1`, paper validation pending `44`, average coverage `0.9833`, `automatic_weight_change_allowed=false`, `broker_submit_allowed=false`, `order_boundary=read_only_no_order`, and `open_gates=[]`. `/api/recommendations/recommendation-67` exposes `professional_evidence_audit.status=source_blocked`, `paper_validation_status=blocked_source`, and `order_boundary=read_only_no_order`. `/data-health` renders `전문 분석 품질`, `추천별 전문 감사`, `재무·피어·밸류에이션·산업·AI 리서치`, `원천 한계 관리 중`, `weight 변경 금지`, and `읽기 전용·주문 금지`; `/recommendations/recommendation-67` renders `추천 전문 분석 감사`, `전문 원천 차단 추천`, `근거 커버리지`, `거래 경계`, and `weight 변경`.
...
1. `outcome-maturity-wait-monitor-v2`: keep the 2026-06-20 and 2026-06-24 outcome windows explicit and route follow-up calibration only when evidence is due.
