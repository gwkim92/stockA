...
1. `valuation-model-quality-depth-v1`: deepen DCF-lite, relative multiple, scenario range, method confidence, and sensitivity assumptions so valuation evidence is analyst-readable before any recommendation weight changes.
...
31. `valuation-model-quality-depth-v1`: next task; DCF-lite, relative multiple, and scenario range evidence should expose explicit assumptions, confidence, sensitivity, and data gaps without changing recommendation scores.
