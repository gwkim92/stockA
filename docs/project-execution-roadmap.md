...
1. `professional-source-gap-remediation-decision-v1`: inspect the live ranked professional source gaps, classify non-remediable free-public-data blockers, and run only safe deterministic remediation commands for remediable gaps.
...
60. `professional-source-gap-remediation-decision-v1`: next task; use the live ranked list to classify/remediate the top true source gap without fabricating financial facts, changing weights, or enabling orders.
