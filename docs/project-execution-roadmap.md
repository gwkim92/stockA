...
| Professional Analysis | Financial normalization, financial statement model detail, peer relative snapshots, financial forecast inputs, sum-of-the-parts valuation foundation, segment/footnote evidence, reported segment parsing, valuation target range, valuation quality depth, portfolio risk budget, position sizing, professional decision waterfall, thesis gates, segment history backfill, segment history coverage expansion, breadth expansion, `aeis-reported-segment-parser-layout-v1`, `segment-history-source-linkage-remediation-v1`, `professional-coverage-refresh-after-source-remediation-v1`, `portfolio-and-fund-instrument-analysis-v1`, `fund-expense-tracking-source-v1`, `fund-expense-ratio-public-source-v1`, and `fund-nav-premium-discount-source-v1` exist. Latest fund analysis evidence is EC2 commit `5073119`: `/api/stocks/SPY` and `/api/recommendations/recommendation-157` expose `fund_instrument_analysis.status=available`, `benchmark_source=ssga_spdr_spy_daily_holdings`, `holding_count=503`, `holdings_coverage_weight=0.9983782`, top holdings `NVDA/AAPL/MSFT/AMZN/GOOGL`, `liquidity.status=collected`, expense ratio `collected` from `ssga_spdr_product_page` with value `0.000945`/`0.094500%`, `nav_premium_discount.status=collected`, NAV `745.571145`, closing price `745.64`, premium/discount `0.0`, NAV source date `2026-05-22`, tracking error `not_collected`, and `order_boundary=read_only_no_order`. Source blockers are still user-facing: SPY/fund-like products show `fund_company_financial_model_not_applicable`, EROK shows `sec_companyfacts_missing_us_gaap_facts`, and ARM remains available without polluted segment labels. | Automatic weight changes remain blocked. Live broker submit remains excluded. Current next work is `fund-tracking-error-source-v1`: add source-backed true tracking error or tracking difference evidence without guessing from one-day NAV premium/discount. |

Latest status update: `fund-nav-premium-discount-source-v1` is complete. EC2 commit `5073119` extends `market.fund_metric_snapshot`, imports SPY NAV, bid/ask midpoint, closing price, and premium/discount from the official State Street SPDR product page with import `run_id=1582`, and renders the source-backed NAV premium/discount evidence on stock and recommendation details. This does not change recommendation weights or broker/order flow.
...
1. `fund-tracking-error-source-v1`: add source-backed true tracking error or tracking difference evidence without hard-coded values, one-day NAV premium/discount guessing, score changes, or order changes.
...
54. `fund-tracking-error-source-v1`: next task; add source-backed true tracking error or tracking difference evidence without one-day NAV premium/discount guessing, recommendation weight changes, or broker/order changes.
