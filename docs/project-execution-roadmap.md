...
| --- | --- | --- |
| Runtime | EC2 FastAPI/Next.js/systemd profile scheduler is the active operating candidate. Production API readiness, read-only RBAC, and free external alert destination are implemented and EC2-smoked. Latest gate evidence: `auth_rbac.status=read_only_rbac_ready`, `alert_destination.status=external_destination_verified`, and `/api/data-health.open_gates=[]`. | Deployment manifest maturity, broader observability history, and production hardening beyond the current EC2 candidate. |
| Data | RSS news, Korean translation, market price, macro, SEC, event enrichment, hierarchical propagation, cycle snapshot, recommendation, paper validation, quality audit, and eval dataset runners are connected through backend CLI runners. Latest `news-intraday` scheduler failure was remediated on EC2 commit `bed027e`; `news-intraday.last_result=success`. AI evidence grounding, stale direct impact, and duplicate title remediation are complete on EC2 commit `278da4a`; latest quality audit is `ok` with `issue_count=0`. `cycle-quality-audit-hardening-v1` is EC2-smoked through commits `d75d5e2`/`ced0d9d`; duplicate titles, ungrounded direct tickers, macro false tickers, and quantum-energy mislinks were all `0`, while normal macro flow samples are visible. | Provider quota resilience and longer-lived outcome history. |
| AI | Codex OAuth batch is used for translation/news extraction and cycle community summaries; deterministic fallback remains available; AI artifacts and invocations are logged. EC2 Codex OAuth re-login and real community summary smoke succeeded on 2026-05-25 with `run_id=712`, `invocation_id=983`, `failed_summary_count=0`. The fixture/gold news AI regression gate is scheduled and visible on data-health; latest EC2 evidence is `news_ai_eval_quality.status=passed`, `eval_run_id=eval-run-44`, `pipeline-run-1701`, `case_count=5`, `failed_case_count=0`. Internal RAG retrieval foundation is implemented without paid vector/graph services; `/api/ai/evidence-neighborhoods/NVDA` returns `internal_rag_context.status=ready`, SQL graph/full-text context, and no live LLM call. AI evidence visibility v3 is EC2-smoked and shows source, Korean translation, AI structure, validator, and recommendation linkage trace. | Longer-horizon AI quality drift history is still needed; current gate covers deterministic regression cases, not multi-week drift. pgvector remains deferred until measured retrieval quality requires it. |
...
| Frontend | Core pages exist and read live backend DTOs. Home/data-health/intelligence/cycle-map/paper-trading now share a “what to review today” decision strip; AI evidence detail has a source-to-recommendation trace path. Home and recommendation list now expose recommendation decision boundaries; `/api/recommendations` latest EC2 smoke returned 9 recommendations, 9 paper-validation-pending candidates, and 9 order-blocked candidates. `/data-health` now exposes professional analysis quality across financial, peer, valuation, industry, AI research, source blockers, and weight/order boundaries. `/recommendations/{id}` now exposes and renders recommendation-level professional evidence audit inline. | Further UX simplification is still possible, but the current blocker is outcome maturity rather than missing recommendation detail evidence. |
...
| Professional Analysis | Financial normalization, financial statement model detail, peer relative snapshots, financial forecast inputs, sum-of-the-parts valuation foundation, segment/footnote evidence, reported segment parsing, valuation target range, valuation quality depth, portfolio risk budget, position sizing, professional decision waterfall, thesis gates, segment history backfill, segment history coverage expansion, breadth expansion, `aeis-reported-segment-parser-layout-v1`, `segment-history-source-linkage-remediation-v1`, `professional-coverage-refresh-after-source-remediation-v1`, ETF/fund evidence, source blocker guardrails, portfolio review feedback, attribution, professional analysis visibility, recommendation-level coverage audit, recommendation detail professional evidence audit, and outcome maturity wait monitor exist. Latest explicit 2026-07-04 evidence says recommendation outcome thresholds reached a review-candidate state while portfolio feedback remained `collect_more_feedback` with `weight_review_blocked=true`; `manual-weight-review-pilot-v1` was not started. `recommendation-weight-review-readiness-semantics-v2` is implemented locally as a non-authoritative, append-only shadow that separates threshold evidence, read-only eligibility, explicit authorization, pilot state, and mutation permissions. Even when legacy thresholds and portfolio feedback are ready, `manual_review_eligible=false` because stable row identity, feedback deduplication, versioned component snapshots, an approved horizon policy, and an approved freshness policy are not attested. It has not been deployed or executed against the live DB. | Automatic weight changes and live broker submit remain blocked. Stable row-level cohort identity, feedback deduplication, versioned component snapshots, an approved horizon policy, an approved freshness policy, scoped pilot parameters, and explicit user authorization are still missing. |
...
Latest follow-up: `portfolio-feedback-calibration-managed-wait-gate-v1` and `professional-analysis-next-action-cockpit-v1` are implemented and EC2-smoked on commit `388034e`. `/api/data-health` now treats immature outcome feedback as a managed wait when the cadence/action router says to wait and all order/weight guardrails remain read-only. Latest evidence: `portfolio_review_feedback_calibration.attention_required=false`, `managed_wait=true`, `managed_gate_status=managed_wait_until_outcome_window`, `weight_review_blocked=true`, `estimated_maturity_date=2026-06-24`, `professional_analysis_next_action.status=managed_outcome_wait`, `next_symbol=EROK`, `order_boundary=read_only_no_order`, `broker_submit_allowed=false`, and `open_gates=[]`. `/data-health` renders `전문 분석 다음 행동`, `전문 분석은 관리 중, 성과 표본 대기`, `관리된 대기`, `왜 open gate가 아닌가`, and `weight 변경 금지`. This is gate classification and visibility only; recommendation weights, benchmark composition, portfolio positions, and broker/order flow remain unchanged.

Latest operational gate update: `alert-destination-free-channel-v1` is implemented and EC2-smoked on commit `56dff0d`. A repo-outside free `ntfy` destination and status artifact are configured without printing the URL. `/api/data-health` now exposes `alert_destination.status=external_destination_verified`, `attention_required=false`, `mode=ntfy`, `destination_type=ntfy`, `target_configured=true`, `last_test_status=passed`, and `test_recent=true`; `alert_destination` is removed from `open_gates`. The previous `auth-rbac-readonly-boundary-v1` evidence remains active with `auth_rbac.status=read_only_rbac_ready`, `attention_required=false`, `read_role=viewer`, and `order_boundary=read_only_no_order`. The previous `production-api-server-gate-evidence-v1` evidence remains active with `production_api_server.status=production_ready`, `attention_required=false`, `runtime_profile=production`, `source_mode=live`, `auth_mode=read-token`, and `connection_boundary=psycopg_pool`. As of commit `4f79661`, there are no open gates in `/api/data-health`; immature outcome feedback remains visible as managed wait and still blocks recommendation weight changes.

Latest evidence/visibility update: `internal-rag-retrieval-foundation-v1`, `ai-evidence-visibility-v3`, `cycle-quality-audit-hardening-v1`, `decision-cockpit-recommendation-boundary-summary-v1`, `professional-analysis-quality-v1`, `professional-recommendation-coverage-audit-v1`, `recommendation-detail-professional-evidence-v2`, and `outcome-maturity-wait-monitor-v2` are implemented and EC2-smoked. Latest commits are `084010b`/`bfa11d2` for internal RAG, `3b4c2c9`/`17deba7` for AI evidence visibility, `ced0d9d`/`d75d5e2` for cycle quality audit, `56e1e04`/`fa54517` for recommendation boundary summary, `4f79661`/`87244ae` for professional analysis quality, `837a5b2` for recommendation-level professional coverage audit, `caca2d9`/`0fefb50` for recommendation detail professional evidence audit, and `88a233d` for outcome maturity wait monitor. `/api/data-health` exposes `professional_analysis_quality.status=managed_source_limited`, active candidates `23`, complete candidates `22`, source blocked `1`, average coverage `0.9674`, `professional_recommendation_coverage_audit.status=source_limited`, recommendation rows `45`, source blocked `1`, paper validation pending `44`, average coverage `0.9833`, `outcome_maturity_wait_monitor.status=managed_wait`, recommendation next due `2026-06-20`, portfolio feedback maturity `2026-06-24`, `automatic_weight_change_allowed=false`, `broker_submit_allowed=false`, `order_boundary=read_only_no_order`, and `open_gates=[]`. `/api/recommendations/recommendation-67` exposes `professional_evidence_audit.status=source_blocked`, `paper_validation_status=blocked_source`, and `order_boundary=read_only_no_order`. `/data-health` renders `성과 성숙 대기 모니터`, `추천 outcome`, `포트폴리오 feedback`, `2026-06-20`, `2026-06-24`, `weight 검토`, `자동 변경 금지`, `전문 분석 품질`, and `추천별 전문 감사`; `/recommendations/recommendation-67` renders `추천 전문 분석 감사`, `전문 원천 차단 추천`, `근거 커버리지`, `거래 경계`, and `weight 변경`.
...
- `auth-rbac-readonly-boundary-v1`: implemented and EC2-smoked on commit `84e2cff`; `/api/data-health.auth_rbac.status=read_only_rbac_ready`, write methods are blocked, and broker submit remains false.
- `alert-destination-free-channel-v1`: implemented and EC2-smoked on commit `56dff0d`; repo-outside free `ntfy` alert destination test passed without exposing the destination URL.
- `internal-rag-retrieval-foundation-v1`: implemented and EC2-smoked on commit `bfa11d2`; internal RAG context is built from Postgres graph/full-text evidence without paid external services or live request-time LLM calls.
- `ai-evidence-visibility-v3`: implemented and EC2-smoked on commit `17deba7`; AI evidence detail now traces source, Korean translation, AI structure, validator result, and recommendation linkage.
- `cycle-quality-audit-hardening-v1`: implemented and EC2-smoked on commit `d75d5e2`; contamination checks are visible in data-health and normal macro-flow samples are separated from missing-ticker errors.
- `decision-cockpit-recommendation-boundary-summary-v1`: implemented and EC2-smoked on commit `fa54517`; home and recommendations list expose decision/paper/order boundaries.
- `professional-analysis-quality-v1`: implemented and EC2-smoked on commit `87244ae`; data-health exposes active recommendation professional-analysis quality and keeps weight/order boundaries blocked.
- `professional-recommendation-coverage-audit-v1`: implemented and EC2-smoked on commit `837a5b2`; data-health exposes recommendation-level professional evidence coverage, paper-validation status, source blockers, and read-only order boundary.
- `recommendation-detail-professional-evidence-v2`: implemented and EC2-smoked on commits `caca2d9`/`0fefb50`; recommendation detail exposes `professional_evidence_audit`, layer checks, source blocker, paper validation status, and read-only weight/order boundary.
- `outcome-maturity-wait-monitor-v2`: implemented and EC2-smoked on commit `88a233d`; data-health exposes the combined recommendation outcome and portfolio feedback maturity wait state with 2026-06-20/2026-06-24 dates and keeps weight/order boundaries blocked.
...
- `recommendation-weight-review-readiness-semantics-v2`: implemented locally as `mode=shadow_read_only`, `authoritative=false`; it snapshots v1 readiness/quality/outcome/portfolio-feedback lineage and 30/90/180/365 horizon evidence, allows legacy `threshold_evidence_ready` to be reported while keeping `manual_review_eligible=false` until all integrity policies are attested, keeps user approval and every mutation/order permission false, writes only `ops.pipeline_run` plus append-only `ai.eval_run`, and adds an isolated allowlisted data-health sibling without changing current gates.

Current remaining sequence after roadmap sync:

1. Deploy and execute `recommendation-weight-review-readiness-semantics-v2` only as a shadow audit, then compare it with the existing v1 artifacts; do not cut over any current gate.
2. Add a prospective cohort/component snapshot, feedback-deduplication foundation, and explicit freshness policy so future outcome evidence has stable row identity, versioned component inputs, and auditable age bounds.
3. Define the authoritative horizon policy, component scope, maximum proposed delta, evaluation window, rollback rule, and production-non-application rule in a separate approval packet.
4. Start `manual-weight-review-pilot-v1` only after the above evidence is verified and the user gives exact explicit approval. Until then all recommendation weights remain unchanged.
