...
| Professional Analysis | Financial normalization, financial statement model detail, peer relative snapshots, financial forecast inputs, sum-of-the-parts valuation foundation, segment/footnote evidence, reported segment parsing, valuation target range, valuation quality depth, portfolio risk budget, position sizing, professional decision waterfall, thesis gates, segment history backfill, segment history coverage expansion, breadth expansion, `aeis-reported-segment-parser-layout-v1`, `segment-history-source-linkage-remediation-v1`, `professional-coverage-refresh-after-source-remediation-v1`, `portfolio-and-fund-instrument-analysis-v1`, and `fund-expense-tracking-source-v1` exist. Latest fund analysis evidence is EC2 commit `450d5e9`: `/api/stocks/SPY` and `/api/recommendations/recommendation-157` expose `fund_instrument_analysis.status=available`, `benchmark_source=ssga_spdr_spy_daily_holdings`, `holding_count=503`, `holdings_coverage_weight=0.9983782`, top holdings `NVDA/AAPL/MSFT/AMZN/GOOGL`, `liquidity.status=collected`, `liquidity.source_name=market.daily_price_bar`, `liquidity.observation_count=100`, expense ratio `not_collected`, tracking error `not_collected`, and `order_boundary=read_only_no_order`. Source blockers are still user-facing: SPY/fund-like products show `fund_company_financial_model_not_applicable`, EROK shows `sec_companyfacts_missing_us_gaap_facts`, and ARM remains available without polluted segment labels. | Automatic weight changes remain blocked. Live broker submit remains excluded. Current next work is `fund-expense-ratio-public-source-v1`: add a free auditable public expense-ratio source path without guessing constants. |

Latest status update: `fund-expense-tracking-source-v1` is complete. EC2 commit `450d5e9` keeps SPY holdings analysis from SSGA, adds source-backed liquidity from `market.daily_price_bar`, and leaves expense ratio plus tracking error/NAV drift as explicit unknowns because no auditable source is currently stored. This does not change recommendation weights or broker/order flow.
...
1. `fund-expense-ratio-public-source-v1`: add a free auditable public expense-ratio source path for ETF/fund analysis without hard-coded constants or score/order changes.
...
52. `fund-expense-ratio-public-source-v1`: next task; add a free auditable public expense-ratio source path without hard-coded constants, recommendation weight changes, or broker/order changes.
