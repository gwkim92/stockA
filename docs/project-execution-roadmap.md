...
1. `portfolio-risk-budget-drift-quality-audit`: track benchmark composition coverage, stale holdings, partial composition warnings, and drift outliers in data-health.
2. `portfolio-risk-budget-full-holdings-source`: replace partial operator smoke file with a full free provider holdings file or broader operator-upload holdings set.
...
22. `portfolio-risk-budget-drift-quality-audit`: next task; expose benchmark composition coverage and stale/partial drift quality in data-health.
