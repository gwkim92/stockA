...
1. `reported-segment-footnote-parser-v1`: parse real SEC segment footnote metrics into `reported_segment_metric` rows when source documents contain segment tables.
...
35. `reported-segment-footnote-parser-v1`: next task; parse actual SEC segment tables/footnotes into `reported_segment_metric` rows instead of relying only on consolidated metrics and explicit gap rows.
