...
Latest operational gate update: `production-api-server-gate-evidence-v1` is implemented and EC2-smoked on commit `e238663`. `/api/data-health` now exposes `production_api_server.status=production_ready`, `attention_required=false`, `runtime_profile=production`, `source_mode=live`, `auth_mode=read-token`, and `connection_boundary=psycopg_pool`. The static `production_api_server` open gate is removed when this evidence is present. The previous `data-operations-artifact-runner-gate-evidence-v1` evidence remains active with `data_operations_artifact_runner.status=operational_profile_scheduler_active`, `artifact_policy_count=33`, `job_count=33`, `latest_run_count=33`, and `active_timer_count=7/timer_count=7`. Remaining open gates are `auth_rbac`, `alert_destination`, and `portfolio_review_feedback_calibration_attention`.
