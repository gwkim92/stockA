...
Latest status update: `portfolio-attribution-monthly-profile-integration-v1` is implemented and EC2-smoked on commit `4809684`. Current live result is `portfolio-attribution-monthly.latest_run_id=pipeline-run-1706`, `latest_status=succeeded`, `health_status=ok`, direct runner `run_id=1704`, `attribution_run_id=1`, selected snapshot `2026-05-22`, selected measurement end `2026-05-22`, and covered symbol `NVDA`. Recommendation outcome due action remains waiting until `2026-06-20`; recommendation weights and broker submit remain blocked.
