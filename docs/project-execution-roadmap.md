...
| Professional Analysis | Financial normalization, financial statement model detail, peer relative snapshots, financial forecast inputs, sum-of-the-parts valuation foundation, segment/footnote evidence, reported segment parsing, valuation target range, valuation quality depth, portfolio risk budget, position sizing, professional decision waterfall, thesis gates, segment history backfill, segment history coverage expansion, breadth expansion, `aeis-reported-segment-parser-layout-v1`, `segment-history-source-linkage-remediation-v1`, `professional-coverage-refresh-after-source-remediation-v1`, ETF/fund evidence, source blocker guardrails, portfolio review feedback, attribution, professional analysis visibility, recommendation-level coverage audit, recommendation detail professional evidence audit, and outcome maturity wait monitor exist. `recommendation-weight-review-readiness-semantics-v2` is deployed at EC2 commit `83e28638` as a non-authoritative, append-only shadow. Live pipeline `10991`/eval `809` returned `evidence_incoherent_fail_closed`: readiness `28` references quality/outcome `26`/`27`, while latest selection is `801`/`692`, and outcome `692` lacks required cohort-filter/nested-quality identity. v1 and `open_gates` hashes were unchanged. `manual_review_eligible=false`, every mutation/order/broker permission is false, and `manual-weight-review-pilot-v1` remains not started. | Automatic weight changes and live broker submit remain blocked. Canonical source-lineage selection, stable row-level cohort identity, feedback deduplication, versioned component snapshots, an approved horizon policy, an approved freshness policy, scoped pilot parameters, and explicit user authorization are still missing. |
...
- `recommendation-weight-review-readiness-semantics-v2`: implemented and EC2-smoked on commit `83e28638`; live pipeline `10991`/eval `809` is `evidence_incoherent_fail_closed`, preserves `manual_review_eligible=false` and `read_only_no_order`, and leaves existing v1/open-gate canonical hashes unchanged.

Current remaining sequence after roadmap sync:

1. Add `recommendation-weight-review-source-lineage-reconciliation-v1` as a read-only task that defines one canonical readiness→quality→outcome chain plus versioned cohort-filter and nested-quality identity; do not rewrite legacy evidence to make v2 green.
