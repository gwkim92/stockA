...
1. `portfolio-position-sizing-policy-v1`: convert professional analysis inputs and benchmark risk into position sizing review envelopes without changing recommendation weights or enabling broker orders.
...
25. `portfolio-position-sizing-policy-v1`: next task; build review-only position sizing envelopes from thesis quality, valuation margin, active risk, liquidity/cash buffer, and concentration constraints.
