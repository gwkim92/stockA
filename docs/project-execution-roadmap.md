...
14. `paper-safety-interlock-policy`: decide whether `ready_for_weight_review` should remain blocked by intentional paper-only safety interlocks, and if not, add a separate gate that preserves live trading prohibition while allowing manual weight review.
