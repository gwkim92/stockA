...
67. `portfolio-review-decision-outcome-feedback-v1`: implemented locally; saved portfolio review decisions are checked against later paper validation, recommendation outcome, thesis, and price evidence, persisted as audit-only `ai.eval_run` feedback, and exposed on data-health plus portfolio coverage without changing weights or orders.
68. `portfolio-review-feedback-calibration-v1`: planned as the next task; accumulated feedback artifacts should be calibrated over time before any future manual weight-pilot readiness.
