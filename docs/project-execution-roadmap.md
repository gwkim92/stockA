...
- `valuation-target-range-foundation-v1`: implemented locally; `/api/stocks/{symbol}`, `/api/recommendations/{id}`, and `/api/theses/{id}` now expose `valuation_target_range`, and the matching pages render current price, target low/base/high, upside, margin-of-safety, and method evidence without score or order changes.

Current remaining task:

1. `financial-statement-model-detail-v1`: expose analyst-style financial statement model detail, including revenue, margins, cash flow, balance sheet risk, dilution, and earnings quality evidence, without changing recommendation weights or enabling broker orders.
...
28. `valuation-target-range-foundation-v1`: implemented locally; valuation snapshots now become explicit target range evidence with conservative scenario assumptions, upside/downside, and margin-of-safety visibility, still with zero recommendation weight changes.
29. `financial-statement-model-detail-v1`: next task; expose the normalized financial model as analyst-readable statement trends and earnings-quality evidence before any score weight changes.
