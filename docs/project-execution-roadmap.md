...
Latest operational gate update: `alert-destination-readiness-visibility-v1` is implemented and EC2-smoked on commit `dad38e4`. `/api/data-health` now exposes `alert_destination.status=missing_destination`, `attention_required=true`, `mode=missing`, `target_configured=false`, and `last_test_status=missing`; the `alert_destination` open gate intentionally remains open until an external destination and recent passed test artifact exist. `/data-health` renders `운영 알림 확인 필요`, `알림 목적지`, `외부 알림 목적지가 설정되지 않았다`, and `알림 설정 보기`. The previous `production-api-server-gate-evidence-v1` evidence remains active with `production_api_server.status=production_ready`, `attention_required=false`, `runtime_profile=production`, `source_mode=live`, `auth_mode=read-token`, and `connection_boundary=psycopg_pool`, so the static `production_api_server` gate stays closed. Remaining open gates are `auth_rbac`, `alert_destination`, and `portfolio_review_feedback_calibration_attention`.
