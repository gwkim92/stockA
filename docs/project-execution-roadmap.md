...
- `thesis-lifecycle-professional-gates-v1`: implemented; `/api/theses/{id}` now exposes `professional_lifecycle_gates` and evidence `observed_at`, and `/theses/{id}` renders the professional thesis gates across buy case, catalysts, risks, invalidation, valuation, review cadence, evidence freshness, and order boundary without score or order changes.

Current remaining task:

1. `valuation-target-range-foundation-v1`: make valuation snapshots expose a conservative target range, base/bull/bear assumptions, upside/downside, and margin-of-safety evidence in API and frontend without changing recommendation weights or enabling broker orders.
...
28. `valuation-target-range-foundation-v1`: next task; turn valuation snapshots into explicit target range evidence with conservative scenario assumptions, upside/downside, and margin-of-safety visibility, still with zero recommendation weight changes.
