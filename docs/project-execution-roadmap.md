...
| Professional Analysis | Financial normalization, financial statement model detail, peer relative snapshots, financial forecast inputs, sum-of-the-parts valuation foundation, segment/footnote evidence, reported segment parsing, valuation target range, valuation quality depth, portfolio risk budget, position sizing, professional decision waterfall, thesis gates, segment history backfill, segment history coverage expansion, breadth expansion, `aeis-reported-segment-parser-layout-v1`, `segment-history-source-linkage-remediation-v1`, and `professional-coverage-refresh-after-source-remediation-v1` exist. Latest professional coverage refresh evidence is EC2 coverage refresh `run_id=1519`, post-decision refresh `run_id=1565`, recommendation component rerun `run_id=1579`, and quality eval `run_id=1580`/`eval_run_id=25`, which reports `quality_status=ready_for_weight_review`, professional coverage `39/45 = 0.866667`, outcome count `30`, `recommendation_scoring_mutated=false`, `automatic_order_allowed=false`, `broker_submit_allowed=false`, `order_boundary=read_only_no_order`. Source blockers are now user-facing: SPY/fund-like products show `fund_company_financial_model_not_applicable`, EROK shows `sec_companyfacts_missing_us_gaap_facts`, and ARM remains available without polluted segment labels. | Automatic weight changes remain blocked. Live broker submit remains excluded. Current next work is `portfolio-and-fund-instrument-analysis-v1`: analyze ETF/fund-like instruments through holdings, benchmark composition, tracking error, exposure, and expense ratio rather than forcing company financial models. |

Latest status update: `professional-coverage-refresh-after-source-remediation-v1` is complete. EC2 refreshed professional coverage after source cleanup with coverage refresh `run_id=1519`, post-decision refresh `run_id=1565`, recommendation component rerun `run_id=1579`, and quality eval `run_id=1580`/`eval_run_id=25`. Quality status is `ready_for_weight_review`, professional coverage is `39/45 = 0.866667`, and outcome count is `30`, but recommendation weights and broker/order flow remain unchanged. Route smoke confirmed SPY/fund-like products show a fund-specific financial-model blocker, EROK shows a SEC companyfacts source blocker, and ARM financial evidence no longer exposes polluted segment labels.
...
1. `portfolio-and-fund-instrument-analysis-v1`: add a professional analysis lane for ETF/fund-like instruments such as SPY so they are judged through holdings, benchmark composition, tracking error, exposure, expense ratio, liquidity, and portfolio role instead of company financial statement models.
...
50. `portfolio-and-fund-instrument-analysis-v1`: next task; create the ETF/fund professional analysis lane for SPY-like instruments using holdings, benchmark composition, tracking error, exposure, expense ratio, liquidity, and portfolio role instead of company financial models.
