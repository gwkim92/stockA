...
1. `segment-level-sotp-valuation-allocation-v1`: use reported segment inputs to derive explicit segment-level valuation allocation evidence without changing recommendation weights.
...
38. `segment-level-sotp-inputs-v1`: next task; use reported segment revenue and operating income evidence as explicit SOTP input/visibility without changing recommendation weights.
