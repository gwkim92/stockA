...
| Professional Analysis | Financial normalization, financial statement model detail, peer relative snapshots, financial forecast inputs, sum-of-the-parts valuation foundation, segment/footnote evidence, reported segment parsing, valuation target range, valuation quality depth, portfolio risk budget, position sizing, professional decision waterfall, thesis gates, segment history backfill, segment history coverage expansion, breadth expansion, `aeis-reported-segment-parser-layout-v1`, `segment-history-source-linkage-remediation-v1`, `professional-coverage-refresh-after-source-remediation-v1`, `portfolio-and-fund-instrument-analysis-v1`, `fund-expense-tracking-source-v1`, and `fund-expense-ratio-public-source-v1` exist. Latest fund analysis evidence is EC2 commit `b8f6e76`: `/api/stocks/SPY` and `/api/recommendations/recommendation-157` expose `fund_instrument_analysis.status=available`, `benchmark_source=ssga_spdr_spy_daily_holdings`, `holding_count=503`, `holdings_coverage_weight=0.9983782`, top holdings `NVDA/AAPL/MSFT/AMZN/GOOGL`, `liquidity.status=collected`, `liquidity.source_name=market.daily_price_bar`, `liquidity.observation_count=100`, expense ratio `collected` from `ssga_spdr_product_page` with value `0.000945`/`0.094500%`, tracking error `not_collected`, and `order_boundary=read_only_no_order`. Source blockers are still user-facing: SPY/fund-like products show `fund_company_financial_model_not_applicable`, EROK shows `sec_companyfacts_missing_us_gaap_facts`, and ARM remains available without polluted segment labels. | Automatic weight changes remain blocked. Live broker submit remains excluded. Current next work is `fund-nav-premium-discount-source-v1`: add source-backed NAV and market-price/NAV premium-discount evidence without guessing tracking error. |

Latest status update: `fund-expense-ratio-public-source-v1` is complete. EC2 commit `b8f6e76` adds `market.fund_metric_snapshot`, imports SPY Gross Expense Ratio from the official State Street SPDR product page, and renders the source-backed `0.094500%` expense ratio on stock and recommendation details. This does not change recommendation weights or broker/order flow.
...
1. `fund-nav-premium-discount-source-v1`: add source-backed NAV and market-price/NAV premium-discount evidence without hard-coded values, tracking-error guessing, score changes, or order changes.
...
53. `fund-nav-premium-discount-source-v1`: next task; add source-backed NAV and market-price/NAV premium-discount evidence without tracking-error guessing, recommendation weight changes, or broker/order changes.
