...
1. `recommendation-financial-model-waterfall-integration-v1`: connect the full financial statement model into recommendation detail and the professional decision waterfall, so “why this recommendation” shows the actual revenue, margin, cash flow, balance sheet, earnings-quality, and dilution evidence behind the financial-quality step.
...
30. `recommendation-financial-model-waterfall-integration-v1`: next task; recommendation detail should reuse the full financial statement model in the professional waterfall instead of relying only on score components.
