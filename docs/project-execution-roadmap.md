...
72. `recommendation-outcome-due-action-router-v1`: implemented locally; new action router runs outcome calibration only when deterministic sample audit reports ready backfill candidates, records no-op/wait or price-gap blocked states otherwise, adds daily cadence/orchestrator integration, exposes the latest router artifact on `/data-health`, and preserves recommendation weight/order guardrails. EC2 smoke is pending.
