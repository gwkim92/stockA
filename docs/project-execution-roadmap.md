...
| Professional Analysis | Financial normalization, financial statement model detail, peer relative snapshots, financial forecast inputs, sum-of-the-parts valuation foundation, segment/footnote evidence, reported segment parsing, valuation target range, valuation quality depth, portfolio risk budget, position sizing, professional decision waterfall, thesis gates, segment history backfill, segment history coverage expansion, breadth expansion, `aeis-reported-segment-parser-layout-v1`, `segment-history-source-linkage-remediation-v1`, `professional-coverage-refresh-after-source-remediation-v1`, `portfolio-and-fund-instrument-analysis-v1`, `fund-expense-tracking-source-v1`, `fund-expense-ratio-public-source-v1`, `fund-nav-premium-discount-source-v1`, `fund-tracking-error-source-v1`, `recommendation-outcome-calibration-sample-expansion-v1`, and `recommendation-weight-review-horizon-gate-v1` exist. Latest weight review horizon-gate evidence is EC2 commit `b3e2915`: `recommendation-weight-review-readiness-audit-run --as-of-date 2026-05-27 --outcome-calibration-eval-run-id 27 --execute` produced `run_id=1598`, `audit_eval_run_id=28`, `source_quality_status=ready_for_weight_review`, `outcome_calibration_status=no_due_outcome_window`, `decision=blocked_by_outcome_calibration_no_due_outcome_window`, `manual_weight_review_allowed=false`, `automatic_weight_change_allowed=false`, `automatic_order_allowed=false`, `broker_submit_allowed=false`, and `recommendation_scoring_mutated=false`. Source blockers are still user-facing: SPY/fund-like products show `fund_company_financial_model_not_applicable`, EROK shows `sec_companyfacts_missing_us_gaap_facts`, and ARM remains available without polluted segment labels. | Automatic weight changes remain blocked. Live broker submit remains excluded. Current next work is `recommendation-outcome-maturity-monitor-v1`: expose when currently not-due recommendation horizons become measurable and what calibration action is next. |

Latest status update: `recommendation-weight-review-horizon-gate-v1` is complete. EC2 commit `b3e2915` makes manual weight review consume the horizon-grid calibration gate. The latest execute result is `run_id=1598`, `audit_eval_run_id=28`, status `blocked_by_outcome_calibration_no_due_outcome_window`: the older source quality eval is `ready_for_weight_review`, but current 30/90/180/365-day outcome windows are still not due, so manual weight review remains blocked. This does not change recommendation weights or broker/order flow.
...
1. `recommendation-outcome-maturity-monitor-v1`: show when currently `not_due` recommendation horizons become measurable and whether outcome backfill/calibration jobs are overdue.
...
57. `recommendation-outcome-maturity-monitor-v1`: next task; expose when currently `not_due` recommendation horizons become measurable and what outcome/calibration action is due next.
