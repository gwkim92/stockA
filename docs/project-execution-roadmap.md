...
- `recommendation-professional-decision-waterfall-v1`: implemented locally; `/api/recommendations/{id}` now exposes `professional_decision_waterfall`, and `/recommendations/{id}` renders that backend decision chain across macro/cycle, news/AI, business/competition, financial quality, valuation, thesis, position sizing, and paper validation without score or order changes.

Current remaining task:

1. `thesis-lifecycle-professional-gates-v1`: make each thesis show and validate why the position exists, what must stay true, what invalidates it, what catalyst/risk/valuation context matters, when it must be reviewed, and which evidence is stale, without changing recommendation weights or enabling broker orders.
...
26. `recommendation-professional-decision-waterfall-v1`: implemented locally; recommendation detail now has an API-backed professional decision chain from macro/cycle/news to company fundamentals, valuation, thesis, position sizing, and paper validation.
27. `thesis-lifecycle-professional-gates-v1`: next task; enforce professional thesis lifecycle gates across catalyst, invalidation, risk, valuation context, evidence freshness, and review cadence.
