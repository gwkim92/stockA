...
Current local follow-up: `portfolio-feedback-calibration-maturity-visibility-v1` is implemented locally and awaiting EC2 smoke. It keeps `portfolio_review_feedback_calibration_attention` open while outcome feedback is immature, but adds maturity status, estimated maturity date, feedback-run gap, mature-decision gap, and a Korean weight-review block reason to data-health and portfolio coverage payloads. This is visibility only; recommendation weights, benchmark composition, portfolio positions, and broker/order flow remain unchanged.
