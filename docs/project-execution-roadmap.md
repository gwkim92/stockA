...
| Professional Analysis | Financial normalization, financial statement model detail, peer relative snapshots, financial forecast inputs, sum-of-the-parts valuation foundation, segment/footnote evidence foundation, reported segment footnote parser foundation, valuation snapshots, valuation target range visibility, industry competitive positioning, equity research artifacts, zero-weight fundamental components, coverage guardrails, active recommendation coverage expansion, `portfolio-risk-budget-policy-v2`, `portfolio-risk-budget-guardrail-run`, `portfolio-risk-budget-paper-validation-integration`, `portfolio-risk-budget-frontend-guardrail-visibility`, `portfolio-risk-budget-benchmark-composition-v1`, `portfolio-risk-budget-benchmark-provider-import-v1`, `portfolio-risk-budget-drift-quality-audit`, `portfolio-risk-budget-full-holdings-source`, `portfolio-risk-budget-rebalance-candidate-review`, `portfolio-position-sizing-policy-v1`, `recommendation-professional-decision-waterfall-v1`, `thesis-lifecycle-professional-gates-v1`, `valuation-target-range-foundation-v1`, `financial-statement-model-detail-v1`, `recommendation-financial-model-waterfall-integration-v1`, `valuation-model-quality-depth-v1`, `financial-forecast-and-scenario-inputs-v1`, `sum-of-the-parts-valuation-foundation-v1`, `segment-footnote-extraction-foundation-v1`, `reported-segment-footnote-parser-v1`, `financial-period-source-document-linkage-v1`, `reported-segment-parser-quality-v1`, `segment-level-sotp-inputs-v1`, `segment-level-sotp-valuation-allocation-v1`, `reported-segment-unit-normalization-v1`, `segment-specific-sotp-assumptions-v1`, and `segment-sotp-driver-calibration-v1`, and `segment-history-backfill-v1` exist. Latest EC2 quality eval reached `ready_for_weight_review` with professional coverage `30/36 = 0.833333` and outcome count `30`. `portfolio-risk-budget-full-holdings-source` imported State Street SPY provider holdings on EC2 with import `run_id=992`, source `ssga_spdr_spy_daily_holdings`, and coverage `0.9983782`; guardrail rerun `run_id=993`, `eval_run_id=23` reports active share `0.77853213`. `portfolio-risk-budget-rebalance-candidate-review` exposes review-only candidates on `/api/trading/readiness` and `/api/portfolio/Long%20Term%20Paper/coverage`, with candidate count `7`, top candidates `TSLA/MSFT/AAPL/NVDA/AMZN`, `automatic_order_allowed=false`, `broker_submit_allowed=false`, and `order_boundary=read_only_no_order`. `portfolio-position-sizing-policy-v1` exposes `position_sizing_review.status=review_required` on EC2 with candidate count `4`, reduce review count `3`, top reduction review candidates `TSLA/MSFT/AAPL`, `NVDA` as hold review, and the same read-only order boundary. `recommendation-professional-decision-waterfall-v1` is EC2-smoked on commit `96bc6db`; `/api/recommendations/recommendation-147` exposes `professional_decision_waterfall.status=paper_validation_required`, step count `8`, `order_boundary=read_only_no_order`, and route `/recommendations/recommendation-147` shows the professional decision chain. `thesis-lifecycle-professional-gates-v1` is EC2-smoked on commit `b2dba33`; `/api/theses/thesis-28` exposes `professional_lifecycle_gates.status=complete`, gate count `8`, evidence `observed_at`, `order_boundary=read_only_no_order`, and route `/theses/thesis-28` shows the thesis gate chain. `valuation-target-range-foundation-v1` adds read-only target range DTO/UI for stocks, recommendations, and theses without score or order changes. `financial-statement-model-detail-v1` is EC2-smoked on commit `9968a5c`; `/api/stocks/AAPL` exposes `financial_statement_model.status=available`, metric count `14`, computed metric count `12`, data gap count `2`, and route `/stocks/AAPL` renders 재무제표 모델, 계산 완료, 이익 품질, and 주식수 변화. `recommendation-financial-model-waterfall-integration-v1` is EC2-smoked on commit `e58cfdb`; `/api/recommendations/recommendation-151` exposes `financial_statement_model.status=available`, metric count `14`, computed metric count `12`, data gap count `2`, latest period `2026-02-20`, `financial_quality.status=재무 모델 연결`, `score_policy=recommendation_weights_unchanged`, `order_boundary=read_only_no_order`, and route `/recommendations/recommendation-151` renders `추천 재무제표 모델`, `계산 완료`, `이익 품질`, and `재무 모델 연결`. `valuation-model-quality-depth-v1` is EC2-smoked on commit `713ae61`; `/api/stocks/NVDA`, `/api/recommendations/recommendation-151`, and `/api/theses/thesis-5` expose `valuation_quality.status=review_required`, method count `3`, data gap count `0`, method sensitivity count `3`, limitations count `2`, and `order_boundary=read_only_no_order`, while routes render `가정 품질` and `모델 한계와 데이터 경고 보기`. `financial-forecast-and-scenario-inputs-v1` is EC2-smoked on commit `b68bbb5`; `financial-forecast-inputs-run` `run_id=1016`/`1027` created `forecast_row_count=285`, `valuation-snapshot-run` `run_id=1017`/`1028` created `snapshot_count=52`, `/api/stocks/NVDA`, `/api/recommendations/recommendation-151`, and `/api/theses/thesis-5` expose forecast evidence `status=available` with DCF/scenario `forecast_row_count=15`, `scenario_count=3`, and routes render `재무 forecast 입력`. `sum-of-the-parts-valuation-foundation-v1` is EC2-smoked on commit `788ade2`; `sum-of-parts-valuation-run` `run_id=1031`/`1033` created `component_row_count=45`, `valuation-snapshot-run` `run_id=1032`/`1034` created `snapshot_count=68` with `sum_of_parts=16`, `/api/stocks/NVDA`, `/api/recommendations/recommendation-151`, and `/api/theses/thesis-5` expose `sotp_evidence.status=available`, `sotp_component_count=3`, and routes render `SOTP 구성요소`; `recommendation_scoring_mutated=false` stayed unchanged. `segment-footnote-extraction-foundation-v1` adds `research.segment_footnote_evidence`, `segment-footnote-evidence-run`, weekly/profile orchestration, and frontend SOTP segment/footnote evidence visibility; local verification proves scoring and order boundaries stay unchanged. `reported-segment-footnote-parser-v1` adds `reported-segment-footnote-parser-run`, simple SEC raw HTML segment table parsing, `reported_segment_metric` upsert, same-period `segment_data_gap` cleanup, and weekly/profile orchestration without score or order changes. `financial-period-source-document-linkage-v1` is EC2-smoked on commit `a2b45f8`; `financial-period-source-linkage-run` `run_id=1042` raw-fetched Apple 10-K source evidence, DB linkage shows `linked_periods=29`, `raw_sec_docs=1`, `linked_raw_periods=2`, and parser follow-up `run_id=1046` now sees `candidate_count=1`. `reported-segment-parser-quality-v1` is EC2-smoked on commit `cdcc1d5`; parser `run_id=1059` produced `reported_segment_metric_count=10`, removed stale period rows, and DB evidence now has AAPL segment rows on `period_end=2025-09-27`. `segment-level-sotp-inputs-v1` is EC2-smoked on commit `705d8a2`; SOTP `run_id=1060` reported `reported_segment_input_count=5`, valuation snapshot `run_id=1061` kept `snapshot_count=68`, `/api/stocks/AAPL` exposes 5 `sotp_evidence.reported_segment_inputs` rows with first segment `Americas`, revenue `178353`, operating income `72480`, operating margin `0.4063850902423845`, `score_policy=recommendation_weights_unchanged`, and `order_boundary=read_only_no_order`; `/stocks/AAPL` renders `사업부별 실적 입력`, `Americas`, and `영업마진`. `segment-level-sotp-valuation-allocation-v1` is EC2-smoked on commit `f0fd075`; SOTP `run_id=1062` and valuation snapshot `run_id=1063` completed with unchanged component and snapshot counts, `/api/stocks/AAPL` exposes 5 `sotp_evidence.reported_segment_allocations` rows, first segment `Americas`, allocation basis `operating_income_share`, allocation weight `0.41257535135504364`, allocated base fair value `57.54485598738951`, allocation sum `1`, `score_policy=recommendation_weights_unchanged`, and `order_boundary=read_only_no_order`; `/stocks/AAPL` renders `사업부별 가치 배분`, `기존 영업사업 SOTP 총액`, and `영업이익 비중`. `reported-segment-unit-normalization-v1` is EC2-smoked on commit `9d5fdfd`; parser `run_id=1064` produced `reported_segment_metric_count=10` with Apple segment metrics normalized to `USD_millions_as_reported`, SOTP `run_id=1065` produced `reported_segment_input_count=5`, valuation `run_id=1066` kept `snapshot_count=68`, `/api/stocks/AAPL` exposes first reported segment input `Americas`, revenue `178353.0`, operating income `72480.0`, operating margin `0.4063850902423845`, `metric_unit=USD_millions_as_reported`, and `/stocks/AAPL` renders `백만 달러 단위`, `사업부별 실적 입력`, `사업부별 가치 배분`, `Americas`, and `영업마진`. `segment-specific-sotp-assumptions-v1` is EC2-smoked on commit `478f49f`; SOTP `run_id=1067` and valuation `run_id=1068` completed with `recommendation_scoring_mutated=false`, `/api/stocks/AAPL` exposes 5 `reported_segment_assumptions`, first assumption `Americas`, `base_growth_rate=0.06`, `base_multiple=20.0`, `driver_label=고마진 현금창출 사업부`, `score_policy=recommendation_weights_unchanged`, `automatic_order_allowed=false`, `broker_submit_allowed=false`, `order_boundary=read_only_no_order`, and `/stocks/AAPL` renders `사업부별 가정`, `성장률·마진·밸류에이션 배수 가정`, `고마진 현금창출 사업부`, and `Americas`. `segment-sotp-driver-calibration-v1` is EC2-smoked on commit `c4dfbb8`; SOTP `run_id=1069` and valuation `run_id=1070` completed with `recommendation_scoring_mutated=false`, `/api/stocks/AAPL` exposes 5 `reported_segment_assumptions`, first assumption `Americas`, `driver_template_label=지역 수요·환율·채널 믹스`, `calibration_method=single_period_margin_share_template_proxy`, `history_period_count=1`, `first_period_end=2025-09-27`, `latest_period_end=2025-09-27`, `observed_revenue_cagr=null`, `observed_margin_change=null`, `base_growth_rate=0.06`, `base_multiple=20.0`, `score_policy=recommendation_weights_unchanged`, `automatic_order_allowed=false`, `broker_submit_allowed=false`, `order_boundary=read_only_no_order`, and `/stocks/AAPL` renders `사업부별 가정`, `동인 지역 수요`, `단일 기간 proxy`, and `Americas`. | Automatic weight changes remain blocked. Live broker submit remains excluded. Current next work is `segment-history-coverage-expansion-v1`: expand segment history coverage from the AAPL proof to active recommendation and portfolio symbols, with explicit unsupported-layout reporting. |

Latest status update: `segment-history-backfill-v1` is now complete and EC2-smoked on commit `a2ad9df`; the authoritative next task is `segment-history-coverage-expansion-v1`.
...
1. `segment-history-coverage-expansion-v1`: expand historical reported segment coverage beyond the AAPL proof to active recommendation and portfolio symbols, with explicit unsupported-layout reporting.
...
44. `segment-history-coverage-expansion-v1`: next task; expand the segment history backfill from the AAPL proof to active recommendation and portfolio coverage, reporting unsupported SEC table layouts explicitly.
