...
Latest status update: `segment-history-coverage-expansion-v1` is now complete and EC2-smoked on commit `f94502c`; the authoritative next task is `reported-segment-parser-layout-expansion-v1`.
...
1. `reported-segment-parser-layout-expansion-v1`: add deterministic parser coverage for the non-AAPL SEC segment layouts surfaced by ADI, without weakening AAPL safeguards.
...
45. `reported-segment-parser-layout-expansion-v1`: next task; inspect ADI raw SEC artifacts and add deterministic parser support for its segment table layout where feasible.
