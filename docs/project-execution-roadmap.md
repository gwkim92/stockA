...
12. `paper-validation-conflict-remediation`: inspect the three current paper validation conflicts, determine whether they are data, thesis, risk-limit, or recommendation-action issues, and keep recommendation weight changes blocked until the conflict source is fixed and paper validation passes.
