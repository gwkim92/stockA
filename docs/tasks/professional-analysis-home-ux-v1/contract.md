...
- [x] `/` 첫 화면에 오늘 결론과 다음 행동 CTA가 명확히 보인다.
- [x] `/`에서 수집, 시장, 뉴스 AI, 사이클, 추천, 거래 안전 순서가 한 화면 흐름으로 보인다.
- [x] 운영 로그성 반복 문구가 줄고, 전문 분석 사이트처럼 보이는 리서치 패킷 구조가 적용된다.
- [x] 검증 명령과 route/browser smoke가 통과한다.
