...
- EC2 smoke found that source-blocked recommendations with existing outcomes could display paper validation as measured. Fixed this so source-blocked recommendations report `paper_validation_status=blocked_source` and the paper validation layer is `blocked`.
