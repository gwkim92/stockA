...
Default Terra and no overrides at revision 2 are active for all five workloads. News translation was switched to Luna through owner Chrome and executed successfully, then restored and executed with Terra. Both CLI-selected model headers match returned transport metadata. Other workloads await new successful observations; their historical DB model labels are displayed without claiming an actual model. The live catalog has seven visible model choices. See `qa.md` and `handoff.md` for deployment, access and evidence.
