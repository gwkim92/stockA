...
- Final EC2 deploy: `develop` fast-forwarded to `8cb83265`, `npm run build` passed, `stockanalysis-frontend-api.service` and `stockanalysis-web.service` were `active`.
- Final EC2 route smoke: panel returned HTTP `200`, collapsed details `[false,false]`, Korean news fallback `true`, non-news valuation detail preserved `true`, order boundary visible `true`, bad panel terms `[]`, overflow `false`.
