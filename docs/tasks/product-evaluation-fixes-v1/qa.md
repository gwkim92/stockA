...
- 최초 전체 CI 34479377494는 `review-workspace.home.ts`의 수정 전 오역 `보안 선택` 기대값 때문에 desktop/mobile 2건이 실패했다. 실제 화면은 올바른 `종목 선택`을 반환했다. 기대값 수정 커밋은 `7a4d46b7`이며 실패했던 2개 검사 재실행이 통과했다.
