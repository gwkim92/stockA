...
- [x] 홈에서 12개 기능 위치가 보인다.
- [x] 데이터 수집 화면이 작업별 상태를 먼저 보여준다.
- [x] 뉴스/AI 화면이 분석 순서를 먼저 보여준다.
- [x] AI 근거 화면이 통과/차단/보류 후보를 분리한다.
- [x] paper 화면이 실제 주문 여부를 명확히 보여준다.
- [x] 로컬 검증이 통과한다.
- [x] EC2 배포와 주요 route smoke가 통과한다.
