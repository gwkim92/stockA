...
- 완료: thesis detail API/화면에 생애주기 DTO를 추가했고, 로컬 계약/타입/빌드/AWH 검증을 통과했다.
...
- exact next step: Git commit/push 후 EC2에 fast-forward 배포하고, `/api/theses/{id}`와 `/theses/{id}`에서 `투자 논리 생애주기`가 렌더링되는지 smoke 검증한다.
