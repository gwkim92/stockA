...
- 무엇을 증명하는가: daily cockpit, remediation tickets, data health, cycle state, recommendation detail, thesis detail, portfolio coverage read DTO contract와 example JSON이 고정되었는지 확인한다.
- 통과 조건: `docs/frontend-api-contract.md`, `docs/api/frontend/contract-index.json`, seven example JSON이 존재하고, contract version과 endpoint/example mapping, common response shape, 핵심 field assertions가 모두 통과하며, root-level `app` scaffold가 없는 것이 확인된다.
