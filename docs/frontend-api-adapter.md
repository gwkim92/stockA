...
- live source는 현재 dashboard, data health, events, theme detail, performance outcomes, remediation tickets, portfolio coverage endpoint를 지원한다.
...
- live source 지원 endpoint는 현재 `GET /api/dashboard/today`, `GET /api/data-health`, `GET /api/events?asOfDate=...`, `GET /api/themes/:themeKey?asOfDate=...`, `GET /api/performance/:portfolio/outcomes?measurementEndDate=...`, `GET /api/remediation-tickets?status=open`, `GET /api/portfolio/:portfolioName/coverage?asOfDate=...`다.
...
1. live support를 recommendation/thesis detail, AI evidence, source document endpoint로 확장한다.
