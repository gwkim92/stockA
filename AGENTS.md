...
- 최신 `professional-source-gap-remediation-decision-v1` 증거는 EC2 commit `7b8fe1a`이다. Decision runner `run_id=1599`, `eval_run_id=29`는 `EROK`을 `non_remediable_current_free_public_data`로 분류했고, `GOOG`를 deterministic SOTP remediation 대상으로 선택했다. GOOG SEC companyfacts rerun `run_id=1601`은 `shares_outstanding`을 포함해 926 facts를 적재했고, SOTP rerun `run_id=1602`는 `component_row_count=60`, `sotp_context_count=22`, `recommendation_scoring_mutated=false`를 반환했다. 최신 `/api/data-health`는 `professional_source_gap_prioritization.status=source_blockers_present`, `gap_count=2`, `source_blocker_count=1`, `coverage_gap_count=0`, `fund_not_applicable_count=1`, top symbols `EROK:source_blocker`, `SPY:fund_not_applicable`, `recommendation_scoring_mutated=false`, `automatic_order_allowed=false`를 반환한다. 다음 작업은 `professional-source-blocker-raw-filing-remediation-v1`로, EROK raw filing/XBRL feasibility를 확인하거나 durable exclusion을 남긴다.
