...
- 최신 운영 gate 작업 `data-operations-artifact-runner-gate-evidence-v1`은 commit `2bdb739`로 구현·EC2 smoke 완료되었다. `/api/data-health`는 `data_operations_artifact_runner.status=operational_profile_scheduler_active`, `artifact_policy_count=33`, `job_count=33`, `latest_run_count=33`, `active_timer_count=7/timer_count=7`을 노출하고, 이 증거가 있으면 정적 `data_operations_artifact_runner` open gate를 제거한다. 현재 남은 open gate는 `production_api_server`, `auth_rbac`, `alert_destination`, `portfolio_review_feedback_calibration_attention`이다.
