...
- 최신 recommendation outcome calibration 증거는 EC2 commit `8c3cbb1`이다. `recommendation-outcome-calibration-sample-expansion-run --execute`는 `run_id=1595`, `eval_run_id=27`, nested backfill `run_id=1596`, nested quality eval `eval_run_id=26`을 만들었고, 결과는 `score_status=no_due_outcome_window`, `recommendation_horizon_count=180`, `recommendation_count=45`, `outcome_count=0`, `not_due_count=180`, `ready_for_backfill_count=0`, `component_diagnostic_count=10`, `recommendation_scoring_mutated=false`, `automatic_order_allowed=false`, `broker_submit_allowed=false`, `order_boundary=read_only_no_order`이다. `/api/data-health`와 `/data-health`는 `추천 성과검증`, `성과 측정일 대기`, `추천 weight를 바꾸기 전에`, `성과 표본`, `주문 경계`를 노출한다. 다음 작업은 기존 manual weight review readiness가 이 horizon-grid gate를 반드시 보게 만드는 `recommendation-weight-review-horizon-gate-v1`이다.
