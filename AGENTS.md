...
- 현재 로컬 후속 작업은 `portfolio-feedback-calibration-maturity-visibility-v1`이다. 이 작업은 남은 `portfolio_review_feedback_calibration_attention`을 닫지 않고, 성숙 예정일, 부족 feedback run 수, 부족 성숙 판단 수, 추천 weight 차단 이유를 `/api/data-health`, `/api/portfolio/{portfolio}/coverage`, `/data-health`에 명확히 노출한다. 추천 scoring weight, benchmark, portfolio position, broker/order flow는 변경하지 않는다.
