...
- 현재 고정된 immediate next task는 `portfolio-risk-budget-rebalance-candidate-review`이다. `professional-equity-analysis-foundation` 흐름에서 재무 정규화, peer relative, valuation snapshot, industry positioning, equity research artifact, zero-weight fundamental components, coverage guardrail, outcome backfill, paper safety interlock, manual weight review report가 구현되었고, 추천 weight 변경은 별도 승인된 pilot-weight task 전까지 금지한다. 포트폴리오 리스크 budget 작업은 `portfolio-risk-budget-policy-v2`, `portfolio-risk-budget-guardrail-run`, `portfolio-risk-budget-paper-validation-integration`, `portfolio-risk-budget-frontend-guardrail-visibility`, `portfolio-risk-budget-benchmark-composition-v1`, `portfolio-risk-budget-benchmark-provider-import-v1`, `portfolio-risk-budget-drift-quality-audit`, `portfolio-risk-budget-full-holdings-source`까지 완료되었다. 최신 EC2 증거는 SSGA SPY provider import `run_id=992`, guardrail rerun `run_id=993`, `eval_run_id=23`, `benchmark_drift.status=calculated`, `benchmark_source=ssga_spdr_spy_daily_holdings`, `source_type=provider_file`, coverage `0.9983782`, active share `0.77853213`, top active outliers `TSLA/MSFT/AAPL`, `/api/data-health` `benchmark_drift_quality.status=drift_outlier_review`이다. EC2에서는 FastAPI/Next.js와 profile별 `systemd` scheduler가 운영 후보로 동작 중이며, `stockanalysis-mvp-20260520`의 현재 public IPv4는 `34.206.72.213`이다. Codex OAuth는 EC2 재로그인 후 실제 `cycle-community-ai-summary-v2-run --provider codex_oauth --execute` smoke가 `run_id=712`, `invocation_id=983`, `failed_summary_count=0`으로 성공했다. 새 data operations 작업은 shell orchestration을 늘리지 말고 `stockanalysis-operations` backend CLI/service boundary를 우선 사용한다. 추천 scoring weight와 실거래 자동화는 별도 승인 전까지 범위 밖이다.
