...
- 현재 고정된 immediate next task는 `portfolio-holding-coverage-remediation`이다. `professional-coverage-expansion-for-active-recommendations`는 EC2에서 active recommendation complete professional coverage를 `30/36 = 0.833333`까지 올렸고, `recommendation-outcome-backfill-run`은 가격 기반 recommendation outcome `55`개와 thesis outcome `43`개를 생성했다. 최신 `recommendation-quality-eval-run`은 `run_id=875`, `eval_run_id=11`, `quality_status=ready_for_weight_review`, `sample_status=sufficient_sample`이다. `recommendation-weight-review-readiness-audit-run`은 EC2에서 `run_id=876`, `audit_eval_run_id=12`, `decision=blocked_by_paper_validation_conflicts`로 통과했고, `paper-validation-conflict-remediation-run`은 EC2에서 `run_id=883`, `decision=blocked_by_portfolio_recommendation_coverage_gap`로 AAPL/MSFT/TSLA를 보유 종목 coverage gap으로 분류했다. AEIS/ARM/QUBT/SPY는 kill switch/human approval safety interlock이다. EC2에서는 FastAPI/Next.js와 profile별 `systemd` scheduler가 운영 후보로 동작 중이며, `stockanalysis-mvp-20260520`의 현재 public IPv4는 `34.206.72.213`이다. Codex OAuth는 EC2 재로그인 후 실제 `cycle-community-ai-summary-v2-run --provider codex_oauth --execute` smoke가 `run_id=712`, `invocation_id=983`, `failed_summary_count=0`으로 성공했다. 새 data operations 작업은 shell orchestration을 늘리지 말고 `stockanalysis-operations` backend CLI/service boundary를 우선 사용한다. 추천 scoring weight와 실거래 자동화는 별도 승인 전까지 범위 밖이다.
