...
- 최신 `cycle-ai-quality-audit-contamination-remediation-v1` 진행 증거는 EC2 commit `9ca5905`이다. `cycle_ai_quality_audit`의 직접 종목 grounding은 normalized company-name token과 curated ETF/index alias(`S&P 500 -> SPY`, `Nasdaq 100 -> QQQ`)를 인정하도록 보정됐고, AI validator도 같은 alias를 허용한다. EC2 `cycle-ai-quality-audit-run --execute --as-of-date 2026-05-26 --lookback-days 30`은 `run_id=1619`, `issue_count=3`, `readiness_gap_count=0`, `ungrounded_direct_ticker_count=1`, `macro_false_ticker_count=1`, `duplicate_title_count=1`, `quantum_energy_mislink_count=0`을 반환했다. 이전 data-health의 `issue_count=12`, `ungrounded_direct_ticker_count=8`, `macro_false_ticker_count=3`, `readiness_gap_count=1`에서 개선됐지만 `/api/data-health`는 아직 `attention_required`이다. 다음 작업은 `event_id=19`/`SPY` stale direct impact cleanup과 duplicate title remediation이다.
