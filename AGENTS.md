...
- 현재 고정된 immediate next task는 professional analysis task group의 `professional-coverage-expansion-for-active-recommendations`이다. `quality-and-evaluation-hardening` task group의 `project-roadmap-reality-sync`, `cycle-ai-e2e-quality-audit`, `news-ai-eval-dataset-and-scoring`, `cycle-community-ai-summary-v2`, `recommendation-quality-calibration`, `decision-cockpit-ux-v2`, `ai-evidence-review-visibility-v2`, `paper-trading-status-clarity`, `recommendation-outcome-backfill`, `codex-oauth-ec2-relogin-smoke`는 구현 또는 smoke 완료 상태다. EC2에서는 FastAPI/Next.js와 profile별 `systemd` scheduler가 운영 후보로 동작 중이며, `stockanalysis-mvp-20260520`의 현재 public IPv4는 `34.206.72.213`이다. Codex OAuth는 EC2 재로그인 후 실제 `cycle-community-ai-summary-v2-run --provider codex_oauth --execute` smoke가 `run_id=712`, `invocation_id=983`, `failed_summary_count=0`으로 성공했다. 전문 분석 레이어는 schema, financial normalization, peer relative, valuation, industry competitive positioning, zero-weight fundamental components, equity research reporting, coverage guardrail까지 구현됐고, 현재 병목은 active recommendation 36개 중 complete professional coverage가 낮은 점이다. 새 data operations 작업은 shell orchestration을 늘리지 말고 `stockanalysis-operations` backend CLI/service boundary를 우선 사용한다. 추천 scoring weight와 실거래 자동화는 별도 승인 전까지 범위 밖이다.
