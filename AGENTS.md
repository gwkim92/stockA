...
- 최신 `source-blocked-recommendation-guardrail-v1` 완료 증거는 EC2 commit `da93536`이다. `/api/recommendations/recommendation-67`은 EROK 추천을 `professional_source_guardrail.status=blocked_by_professional_source_data`, `blocked=true`, `professional_decision_use_allowed=false`, `paper_validation_input_allowed=false`, evidence review `quality_status=blocked`, waterfall `status=source_data_blocked`로 반환한다. `/api/data-health`는 `guarded_source_blocked_recommendation_count=1`, EROK `active_recommendation_professional_use_blocked=true`, `paper_validation_input_allowed=false`를 반환한다. `/`, `/data-health`, `/stocks/EROK`, `/recommendations/recommendation-67` route smoke는 `200`이다. 다음 작업은 `benchmark-drift-outlier-decision-v1`로, benchmark drift outlier를 명시적 검토/리밸런싱 후보 판단으로 연결한다.
