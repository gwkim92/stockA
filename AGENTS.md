...
- 최신 recommendation outcome maturity 증거는 EC2 commit `83f750c`이다. `/api/data-health`는 `recommendation_outcome_maturity.status=not_due`, `next_due_date=2026-06-20`, `next_due_count=19`, `not_due_count=180`, `ready_for_backfill_count=0`, `due_today_count=0`, `overdue_count=0`, `price_gap_count=0`를 반환하며, `recommendation_weight_review_readiness.status=blocked_by_outcome_calibration_no_due_outcome_window`, `manual_weight_review_allowed=false`를 유지한다. `/data-health`는 `성과 측정창`, `다음 측정일`, `지연/가격 보강`, `성과 측정일 대기`를 노출한다. 다음 작업은 maturity state를 실제 cadence/action trigger로 연결하는 `recommendation-outcome-due-cadence-automation-v1`이다.
