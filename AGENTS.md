...
- 최신 ETF/fund analysis 증거는 EC2 commit `5073119`이다. `/api/stocks/SPY`와 `/api/recommendations/recommendation-157`은 `fund_instrument_analysis.status=available`, `benchmark_source=ssga_spdr_spy_daily_holdings`, `holding_count=503`, `holdings_coverage_weight=0.9983782`, top holdings `NVDA/AAPL/MSFT/AMZN/GOOGL`, `liquidity.status=collected`, `liquidity.source_name=market.daily_price_bar`, `liquidity.observation_count=100`, 비용률 `collected`, 비용률 값 `0.000945`/`0.094500%`, 비용률 원천 `ssga_spdr_product_page`, 비용률 기준일 `2026-05-26`, `nav_premium_discount.status=collected`, NAV `745.571145`, closing price `745.64`, premium/discount `0.0`, NAV 기준일 `2026-05-22`, 추적오차 `not_collected`, `order_boundary=read_only_no_order`를 반환한다. EC2 import `run_id=1582`, `fund_metric_snapshot_ids=[2,3,4,5]`이고 `/stocks/SPY`와 `/recommendations/recommendation-157` route smoke는 `NAV 괴리`, `NAV 원천 열기`, `US$745.57`, `0%`, `비용률`, `주문 경계` 문구를 확인했다. 다음 작업은 진짜 multi-period tracking error 또는 tracking difference를 무료 공개 원천에서 가져오거나 explicit unknown으로 유지하는 `fund-tracking-error-source-v1`이다.
