...
- 현재 고정된 immediate next task는 `manual-weight-review-calibration-report`이다. `professional-coverage-expansion-for-active-recommendations`는 EC2에서 active recommendation complete professional coverage를 `30/36 = 0.833333`까지 올렸고, `recommendation-outcome-backfill-run`은 가격 기반 recommendation outcome `55`개와 thesis outcome `43`개를 생성했다. 최신 `recommendation-quality-eval-run`은 `run_id=890`, `eval_run_id=13`, `quality_status=ready_for_weight_review`, `sample_status=sufficient_sample`이다. `portfolio-holding-coverage-remediation`은 EC2에서 active holding thesis를 paper preview coverage로 인정하게 해 AAPL/MSFT/TSLA 오탐 conflict를 해소했고, 새 `paper_validation_run_id=10`은 `conflict_count=0`, `submitted_to_broker_count=0`이다. `paper-safety-interlock-policy`는 EC2에서 `run_id=973`, `audit_eval_run_id=16`, `decision=ready_for_manual_weight_review`로 통과했다. 이 결과는 manual weight review만 허용하며 `automatic_weight_change_allowed=false`, `automatic_order_allowed=false`, `broker_submit_allowed=false`를 유지한다. 다음 작업은 `audit_eval_run_id=16`의 component evidence와 실패 케이스를 사람이 읽는 calibration report로 고정하는 것이며, 추천 weight 변경은 별도 승인된 pilot-weight task 전까지 금지한다. EC2에서는 FastAPI/Next.js와 profile별 `systemd` scheduler가 운영 후보로 동작 중이며, `stockanalysis-mvp-20260520`의 현재 public IPv4는 `34.206.72.213`이다. Codex OAuth는 EC2 재로그인 후 실제 `cycle-community-ai-summary-v2-run --provider codex_oauth --execute` smoke가 `run_id=712`, `invocation_id=983`, `failed_summary_count=0`으로 성공했다. 새 data operations 작업은 shell orchestration을 늘리지 말고 `stockanalysis-operations` backend CLI/service boundary를 우선 사용한다. 추천 scoring weight와 실거래 자동화는 별도 승인 전까지 범위 밖이다.
