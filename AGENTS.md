...
- 최신 ETF/fund analysis 증거는 EC2 commit `b8f6e76`이다. `/api/stocks/SPY`와 `/api/recommendations/recommendation-157`은 `fund_instrument_analysis.status=available`, `benchmark_source=ssga_spdr_spy_daily_holdings`, `holding_count=503`, `holdings_coverage_weight=0.9983782`, top holdings `NVDA/AAPL/MSFT/AMZN/GOOGL`, `liquidity.status=collected`, `liquidity.source_name=market.daily_price_bar`, `liquidity.observation_count=100`, 평균 거래량 `75546352.24`, 평균 거래대금 `51757628999.20085`, 비용률 `collected`, 비용률 값 `0.000945`/`0.094500%`, 비용률 원천 `ssga_spdr_product_page`, 비용률 기준일 `2026-05-26`, 추적오차 `not_collected`, `order_boundary=read_only_no_order`를 반환한다. `/stocks/SPY`와 `/recommendations/recommendation-157` route smoke는 ETF·펀드 비용률/유동성 패널과 read-only boundary 문구를 확인했다. 다음 작업은 NAV와 시장가/NAV premium-discount를 무료 공개 원천에서 가져오거나 explicit unknown으로 유지하는 `fund-nav-premium-discount-source-v1`이다.
