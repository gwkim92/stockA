...
- 현재 고정된 immediate next task는 `recommendation-weight-review-readiness-audit`이다. `professional-coverage-expansion-for-active-recommendations`는 EC2에서 active recommendation complete professional coverage를 `30/36 = 0.833333`까지 올렸고, `recommendation-outcome-backfill-run`은 가격 기반 recommendation outcome `55`개와 thesis outcome `43`개를 생성했다. 최신 `recommendation-quality-eval-run`은 `run_id=875`, `eval_run_id=11`, `quality_status=ready_for_weight_review`, `sample_status=sufficient_sample`이다. 단, 이것은 자동 weight 변경 허가가 아니며, paper validation latest status가 `failed`이고 conflict count가 `3`이므로 `paper-validation-conflict-remediation`도 weight 변경 전 확인해야 한다. EC2에서는 FastAPI/Next.js와 profile별 `systemd` scheduler가 운영 후보로 동작 중이며, `stockanalysis-mvp-20260520`의 현재 public IPv4는 `34.206.72.213`이다. Codex OAuth는 EC2 재로그인 후 실제 `cycle-community-ai-summary-v2-run --provider codex_oauth --execute` smoke가 `run_id=712`, `invocation_id=983`, `failed_summary_count=0`으로 성공했다. 새 data operations 작업은 shell orchestration을 늘리지 말고 `stockanalysis-operations` backend CLI/service boundary를 우선 사용한다. 추천 scoring weight와 실거래 자동화는 별도 승인 전까지 범위 밖이다.
