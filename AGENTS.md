...
- 현재 고정된 immediate next task는 `frontend-api-server-observability-hardening`이다. FastAPI read-only server 이후에는 새 프론트 기능보다 request id, timeout, structured log, readiness 같은 운영 hardening을 먼저 진행한다.
