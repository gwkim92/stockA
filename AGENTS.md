...
- 최신 `cycle-ai-quality-audit-contamination-remediation-v1` 완료 증거는 EC2 commit `278da4a`이다. 직접 종목 grounding은 normalized company-name token과 curated ETF/index alias(`S&P 500 -> SPY`, `Nasdaq 100 -> QQQ`)를 인정하도록 보정됐고, AI validator도 같은 alias를 허용한다. Stale direct impact cleanup은 `run_id=1620`으로 `event_id=19`/`SPY` 한 건을 제거했고, duplicate title cleanup은 `run_id=1622`로 `SpaceX's road to landmark IPO filing` 중복 문서/빈 이벤트 한 건을 제거했다. 최신 `cycle-ai-quality-audit-run --execute --as-of-date 2026-05-26 --lookback-days 30`은 `run_id=1623`, `audit_status=ok`, `audit_score=100`, `issue_count=0`, `readiness_gap_count=0`, `ungrounded_direct_ticker_count=0`, `macro_false_ticker_count=0`, `duplicate_title_count=0`, `quantum_energy_mislink_count=0`을 반환한다. `/`, `/data-health`, `/intelligence`, `/stocks/EROK` route smoke는 `200`이다. 다음 작업은 `source-blocked-recommendation-guardrail-v1`로, EROK처럼 durable source blocker가 있는 종목의 active recommendation을 전문 판단 입력으로 쓰지 못하게 막는다.
