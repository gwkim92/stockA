...
- 최신 local `professional-source-gap-prioritization-v1` 구현은 `/api/data-health`에 `professional_source_gap_prioritization`을 추가하고 `/data-health`에 `전문 분석 소스 공백` 섹션을 추가했다. EROK 같은 `sec_companyfacts_missing_us_gaap_facts`는 true company source blocker로, SPY 같은 `fund_company_financial_model_not_applicable`은 ETF/fund company-model not-applicable로 분리한다. 다음 작업은 EC2 배포/smoke 후 live ranked gap의 top deterministic remediation command만 실행하는 것이다.
