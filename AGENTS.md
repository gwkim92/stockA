...
- 최신 `benchmark-drift-outlier-decision-v1` 로컬 증거는 `/api/data-health` `benchmark_drift_quality.outlier_decisions`와 `/api/portfolio/{portfolio}/coverage` `risk_budget.rebalance_candidate_review.candidates`가 `review_decision`, `decision_label`, `next_review_action`, source evidence, related thesis/recommendation context when present, `automatic_order_allowed=false`, `broker_submit_allowed=false`, `order_boundary=read_only_no_order`를 반환하는 것이다. 다음 작업은 `portfolio-review-decision-history-v1`로, 현재 read-time DTO derivation인 포트폴리오 검토 결정을 durable audit history로 남긴다.
