...
CONTAINER_NAME="${RECOMMENDATION_BOOTSTRAP_VERIFY_CONTAINER_NAME:-stockanalysis-recommendation-bootstrap-verify}"
POSTGRES_IMAGE="${RECOMMENDATION_BOOTSTRAP_VERIFY_POSTGRES_IMAGE:-postgres:16-alpine}"
POSTGRES_DB="${RECOMMENDATION_BOOTSTRAP_VERIFY_POSTGRES_DB:-stockanalysis}"
POSTGRES_USER="${RECOMMENDATION_BOOTSTRAP_VERIFY_POSTGRES_USER:-postgres}"
POSTGRES_PASSWORD="${RECOMMENDATION_BOOTSTRAP_VERIFY_POSTGRES_PASSWORD:-postgres}"
ARTIFACT_DIR=$(mktemp -d /tmp/stockanalysis-recommendation-bootstrap.XXXXXX)
...
batch_count=$(docker exec "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Atc \
  "select count(*) from signal.recommendation_batch where as_of_date = '2024-11-01' and market_code = 'US' and strategy_name = 'long_term_core' and horizon_type = 'long_term';")
...
aapl_recommendation_count=$(docker exec "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Atc \
  "select count(*) from signal.recommendation recommendation join signal.recommendation_batch batch on batch.batch_id = recommendation.batch_id join ref.instrument instrument on instrument.instrument_id = recommendation.instrument_id where batch.as_of_date = '2024-11-01' and instrument.primary_symbol = 'AAPL' and recommendation.rank_position = 1 and recommendation.bucket = 'watch' and recommendation.action = 'watch' and recommendation.total_score = 0.3610;")
source_run_linked_count=$(docker exec "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Atc \
  "select count(*) from signal.recommendation_batch where as_of_date = '2024-11-01' and source_run_id is not null and notes like '%bootstrap-v1%';")
...
test "$aapl_recommendation_count" = "1"
