...
CONTAINER_NAME="${SEC_EVENT_BATCH_VERIFY_CONTAINER_NAME:-stockanalysis-sec-event-batch-verify}"
POSTGRES_IMAGE="${SEC_EVENT_BATCH_VERIFY_POSTGRES_IMAGE:-postgres:16-alpine}"
POSTGRES_DB="${SEC_EVENT_BATCH_VERIFY_POSTGRES_DB:-stockanalysis}"
POSTGRES_USER="${SEC_EVENT_BATCH_VERIFY_POSTGRES_USER:-postgres}"
POSTGRES_PASSWORD="${SEC_EVENT_BATCH_VERIFY_POSTGRES_PASSWORD:-postgres}"
ARTIFACT_DIR=$(mktemp -d /tmp/stockanalysis-sec-event-batch.XXXXXX)
...
linked_event_count=$(docker exec "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Atc \
  "select count(*) from event.event e join event.event_document_link l on l.event_id = e.event_id join ingest.source_document d on d.document_id = l.document_id where d.external_document_id in ('0000320193-24-000123','0000320193-24-000101') and l.link_type = 'source';")
annual_count=$(docker exec "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Atc \
  "select count(*) from event.event where dedupe_key = 'sec_edgar:0000320193-24-000123:sec_annual_report_filed';")
quarterly_count=$(docker exec "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Atc \
  "select count(*) from event.event where dedupe_key = 'sec_edgar:0000320193-24-000101:sec_quarterly_report_filed';")
run_count=$(docker exec "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Atc \
  "select count(*) from ops.pipeline_run where pipeline_name = 'sec_filings_event_extract' and status = 'succeeded';")

test "$linked_event_count" = "2"
test "$annual_count" = "1"
test "$quarterly_count" = "1"
test "$run_count" = "2"
