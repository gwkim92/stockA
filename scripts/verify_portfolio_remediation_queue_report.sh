...
CONTAINER_NAME="${PORTFOLIO_REMEDIATION_QUEUE_VERIFY_CONTAINER_NAME:-stockanalysis-portfolio-remediation-queue-verify}"
POSTGRES_IMAGE="${PORTFOLIO_REMEDIATION_QUEUE_VERIFY_POSTGRES_IMAGE:-postgres:16-alpine}"
POSTGRES_DB="${PORTFOLIO_REMEDIATION_QUEUE_VERIFY_POSTGRES_DB:-stockanalysis}"
POSTGRES_USER="${PORTFOLIO_REMEDIATION_QUEUE_VERIFY_POSTGRES_USER:-postgres}"
POSTGRES_PASSWORD="${PORTFOLIO_REMEDIATION_QUEUE_VERIFY_POSTGRES_PASSWORD:-postgres}"
ARTIFACT_DIR=$(mktemp -d /tmp/stockanalysis-portfolio-remediation-queue.XXXXXX)
...
queue_json="$ARTIFACT_DIR/portfolio-remediation-queue.json"
STOCKANALYSIS_PSQL_COMMAND="docker exec -i $CONTAINER_NAME psql -U $POSTGRES_USER -d $POSTGRES_DB" \
...
assert payload["report_name"] == "portfolio_remediation_queue", payload
assert payload["portfolio_name"] == "Long Term Paper", payload
assert payload["queue_item_count"] == 1, payload
...
assert item["symbol"] == "BABA", payload
assert item["action"] == "needs_thesis_review", payload
assert item["remediation_type"] == "thesis_remediation", payload
assert item["suggested_runner"] == "thesis_or_position_link_review", payload
assert "Create or link an active thesis" in item["suggested_next_step"], payload
assert "coverage status missing_thesis" in item["reason"], payload
PY
