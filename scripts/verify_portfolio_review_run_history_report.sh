...
CONTAINER_NAME="${PORTFOLIO_REVIEW_RUN_HISTORY_VERIFY_CONTAINER_NAME:-stockanalysis-portfolio-review-run-history-verify}"
POSTGRES_IMAGE="${PORTFOLIO_REVIEW_RUN_HISTORY_VERIFY_POSTGRES_IMAGE:-postgres:16-alpine}"
POSTGRES_DB="${PORTFOLIO_REVIEW_RUN_HISTORY_VERIFY_POSTGRES_DB:-stockanalysis}"
POSTGRES_USER="${PORTFOLIO_REVIEW_RUN_HISTORY_VERIFY_POSTGRES_USER:-postgres}"
POSTGRES_PASSWORD="${PORTFOLIO_REVIEW_RUN_HISTORY_VERIFY_POSTGRES_PASSWORD:-postgres}"
ARTIFACT_DIR=$(mktemp -d /tmp/stockanalysis-portfolio-review-run-history.XXXXXX)
...
report_json="$ARTIFACT_DIR/portfolio-review-run-history.json"
STOCKANALYSIS_PSQL_COMMAND="docker exec -i $CONTAINER_NAME psql -U $POSTGRES_USER -d $POSTGRES_DB" \
...
assert payload["report_name"] == "portfolio_review_run_history", payload
assert payload["portfolio_name"] == "Long Term Paper", payload
assert payload["review_count"] == 1, payload
assert payload["risk_counts"] == {"watch": 1}, payload
assert payload["action_counts"]["monitor"] == 1, payload
assert payload["action_counts"]["needs_thesis_review"] == 1, payload
assert payload["attention_item_count"] == 1, payload
assert payload["latest_review"]["risk_level"] == "watch", payload
assert payload["latest_review"]["item_count"] == 2, payload
assert payload["latest_review"]["attention_item_count"] == 1, payload

review = payload["reviews"][0]
assert review["item_count"] == 2, payload
assert review["action_counts"]["monitor"] == 1, payload
assert review["action_counts"]["needs_thesis_review"] == 1, payload
attention = review["attention_items"]
assert len(attention) == 1, payload
assert attention[0]["symbol"] == "BABA", payload
assert attention[0]["action"] == "needs_thesis_review", payload
assert "coverage status missing_thesis" in attention[0]["reason"], payload
PY
