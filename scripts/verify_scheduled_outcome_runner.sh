...
CONTAINER_NAME="${SCHEDULED_OUTCOME_RUNNER_VERIFY_CONTAINER_NAME:-stockanalysis-scheduled-outcome-runner-verify}"
POSTGRES_IMAGE="${SCHEDULED_OUTCOME_RUNNER_VERIFY_POSTGRES_IMAGE:-postgres:16-alpine}"
POSTGRES_DB="${SCHEDULED_OUTCOME_RUNNER_VERIFY_POSTGRES_DB:-stockanalysis}"
POSTGRES_USER="${SCHEDULED_OUTCOME_RUNNER_VERIFY_POSTGRES_USER:-postgres}"
POSTGRES_PASSWORD="${SCHEDULED_OUTCOME_RUNNER_VERIFY_POSTGRES_PASSWORD:-postgres}"
ARTIFACT_DIR=$(mktemp -d /tmp/stockanalysis-scheduled-outcome-runner.XXXXXX)
...
aapl_short_recommendation_outcome_count=$(docker exec "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Atc \
  "select count(*) from performance.recommendation_outcome outcome join signal.recommendation recommendation on recommendation.recommendation_id = outcome.recommendation_id join ref.instrument instrument on instrument.instrument_id = recommendation.instrument_id where instrument.primary_symbol = 'AAPL' and outcome.measurement_start_date = '2024-11-01' and outcome.measurement_end_date = '2024-11-04' and outcome.horizon_days = 3 and outcome.absolute_return_pct = 0.010000 and outcome.benchmark_code = 'SPY' and outcome.benchmark_return_pct = 0.005000 and outcome.alpha_pct = 0.005000 and outcome.outcome_label = 'outperform';")
aapl_long_recommendation_outcome_count=$(docker exec "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Atc \
  "select count(*) from performance.recommendation_outcome outcome join signal.recommendation recommendation on recommendation.recommendation_id = outcome.recommendation_id join ref.instrument instrument on instrument.instrument_id = recommendation.instrument_id where instrument.primary_symbol = 'AAPL' and outcome.measurement_start_date = '2024-11-01' and outcome.measurement_end_date = '2024-12-02' and outcome.horizon_days = 31 and outcome.absolute_return_pct = 0.100000 and outcome.benchmark_code = 'SPY' and outcome.benchmark_return_pct = 0.040000 and outcome.alpha_pct = 0.060000 and outcome.outcome_label = 'outperform';")
performance_source_run_linked_count=$(docker exec "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Atc \
...
schedule_run_status=$(docker exec "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Atc \
  "select status from ops.pipeline_run where pipeline_name = 'performance_outcome_schedule_bootstrap' order by run_id desc limit 1;")
schedule_run_count=$(docker exec "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Atc \
  "select count(*) from ops.pipeline_run where pipeline_name = 'performance_outcome_schedule_bootstrap' and status = 'succeeded';")
...
test "$performance_source_run_linked_count" = "2"
test "$schedule_run_status" = "succeeded"
test "$schedule_run_count" = "1"
