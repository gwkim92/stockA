...
CONTAINER_NAME="${THESIS_REVIEW_BOOTSTRAP_VERIFY_CONTAINER_NAME:-stockanalysis-thesis-review-bootstrap-verify}"
POSTGRES_IMAGE="${THESIS_REVIEW_BOOTSTRAP_VERIFY_POSTGRES_IMAGE:-postgres:16-alpine}"
POSTGRES_DB="${THESIS_REVIEW_BOOTSTRAP_VERIFY_POSTGRES_DB:-stockanalysis}"
POSTGRES_USER="${THESIS_REVIEW_BOOTSTRAP_VERIFY_POSTGRES_USER:-postgres}"
POSTGRES_PASSWORD="${THESIS_REVIEW_BOOTSTRAP_VERIFY_POSTGRES_PASSWORD:-postgres}"
ARTIFACT_DIR=$(mktemp -d /tmp/stockanalysis-thesis-review-bootstrap.XXXXXX)
...
review_table_count=$(docker exec "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Atc \
  "select count(*) from information_schema.tables where table_schema = 'signal' and table_name = 'thesis_review';")
review_count=$(docker exec "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Atc \
  "select count(*) from signal.thesis_review;")
aapl_review_count=$(docker exec "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Atc \
  "select count(*) from signal.thesis_review review join signal.investment_thesis thesis on thesis.thesis_id = review.thesis_id join ref.instrument instrument on instrument.instrument_id = thesis.instrument_id where instrument.primary_symbol = 'AAPL' and review.review_date = '2024-11-01' and review.review_source = 'deterministic_bootstrap' and review.action = 'watch' and review.health_score = 0.3610 and review.next_review_date = '2024-12-01';")
source_run_linked_count=$(docker exec "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Atc \
  "select count(*) from signal.thesis_review review join ops.pipeline_run run on run.run_id = review.source_run_id where run.pipeline_name = 'thesis_review_bootstrap' and run.status = 'succeeded';")
run_status=$(docker exec "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Atc \
  "select status from ops.pipeline_run where pipeline_name = 'thesis_review_bootstrap' order by run_id desc limit 1;")

test "$review_table_count" = "1"
test "$review_count" = "1"
test "$aapl_review_count" = "1"
