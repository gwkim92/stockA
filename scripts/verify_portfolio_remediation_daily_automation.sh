...
CONTAINER_NAME="${PORTFOLIO_REMEDIATION_DAILY_VERIFY_CONTAINER_NAME:-stockanalysis-portfolio-remediation-daily-verify}"
POSTGRES_IMAGE="${PORTFOLIO_REMEDIATION_DAILY_VERIFY_POSTGRES_IMAGE:-postgres:16-alpine}"
POSTGRES_DB="${PORTFOLIO_REMEDIATION_DAILY_VERIFY_POSTGRES_DB:-stockanalysis}"
POSTGRES_USER="${PORTFOLIO_REMEDIATION_DAILY_VERIFY_POSTGRES_USER:-postgres}"
POSTGRES_PASSWORD="${PORTFOLIO_REMEDIATION_DAILY_VERIFY_POSTGRES_PASSWORD:-postgres}"
ARTIFACT_DIR=$(mktemp -d /tmp/stockanalysis-portfolio-remediation-daily.XXXXXX)
...
daily_json="$ARTIFACT_DIR/portfolio-remediation-daily.json"
...
python3 - "$daily_json" "$daily_run_status" <<'PY'
import json
import sys
...
ticket_bootstrap = payload["ticket_bootstrap"]
assert ticket_bootstrap["ticket_count"] == 1, payload
assert ticket_bootstrap["remediation_type_counts"] == {"thesis_remediation": 1}, payload
assert ticket_bootstrap["action_counts"] == {"needs_thesis_review": 1}, payload
...
assert sys.argv[2].strip() == "succeeded", sys.argv[2]
PY
