...
export function ReviewNotebook({ model, snapshot, source, sourcePanel }: { model: ReviewModel; snapshot: string; source: unknown; sourcePanel: ReactNode }) {
  ...
  return <div className={styles.page} data-testid="company-review-notebook">
    <header className={styles.header}><div><span className={styles.kicker}>RESEARCH NOTEBOOK</span><h1>{model.symbol}<span>검토 노트</span></h1><p>{model.name} · 읽은 근거와 내 판단을 구분해 남깁니다.</p></div><ReaderLink href={`/stocks/${encodeURIComponent(model.symbol)}`}>기업 리서치로 돌아가기 →</ReaderLink></header>
    ...
    <div className={styles.compare}>
      <section id="review-claims" className={styles.analysisPanel} aria-labelledby="review-claims-title">
        <div className={styles.sectionHead}><div><span className={styles.kicker}>01 · STORED ANALYSIS</span><h2 id="review-claims-title">{model.fund ? '노출·비용·제한을 검토하세요' : '주장과 반대 근거를 함께 읽으세요'}</h2></div></div>
