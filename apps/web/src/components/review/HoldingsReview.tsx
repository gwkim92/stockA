...
export function HoldingsReview({ rows }: { rows: readonly Holding[] }) {
  ...
  return <section className={styles.panel} aria-label="보유 종목 검토" data-testid="holdings-review">
    <div className={styles.panelHeading}><h2>보유 종목과 검토 사유</h2><p>투자 논리·성과·평가자료의 공백을 확인하세요.</p></div>
    ...
    {filtered.map(row => <article key={row.id} className={styles.holding}>
      ...
      <div className={styles.reviewReason}><h3>{koCode(row.action)}</h3><p>성과 기록: {koCode(row.outcome)}</p><p>보유 크기: {row.sizeStatus === "within_limit" ? "설정 한도 내" : koCode(row.sizeStatus)}</p>{row.sizeNote && <p>{row.sizeNote}</p>}<div className={styles.links}><Link href={`/stocks/${encodeURIComponent(row.symbol)}` as Route}>기업 분석 →</Link>{row.thesisId && <Link href={`/theses/${encodeURIComponent(row.thesisId)}` as Route}>투자 논리 →</Link>}</div></div>
    </article>)}
