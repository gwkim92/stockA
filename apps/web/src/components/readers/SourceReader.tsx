...
export function SourceReader({ data }: { data: SourceReaderData }) {
  ...
  return <ReaderFrame eyebrow="SOURCE READER · 원천 대조" title={data.koreanTitle ?? data.title}
    subtitle={`${data.publisher} · ${data.integrityNotice ? "원천 식별자 불일치" : data.form || koCode(data.type)} · 공개일 ${shortDate(data.filedAt, "미기록")}`}
    chapters={[["source-excerpts", "발췌 읽기"], ["source-summary", "요약·원제"], ["source-connections", "근거 연결"], ["source-record", "수집 기록"]]}
    aside={<>
      <section className={styles.contextCard}><h2>문서 맥락</h2><ReaderFacts items={[["연결 종목", data.symbol ?? "미확인"], ["원천 유형", data.integrityNotice ? "원천 식별자 불일치" : koCode(data.type)], ["기록된 대상일", data.periodEnd ?? "미기록"], ["게시·접수일", shortDate(data.filedAt, "미기록")], ["수집일", shortDate(data.fetchedAt, "미기록")]]} /><p className={styles.caption}>수집일은 원문 작성일·최신성을 뜻하지 않습니다.</p><ReaderLink href={route("stocks", data.symbol)}>연결 기업 리서치 →</ReaderLink><ReaderLink href={data.thesisHref}>연결 투자 논리 →</ReaderLink></section>
    ...
    {data.integrityNotice && <p role="status" className={styles.contextCard}><strong>{data.integrityNotice}</strong></p>}
    ...
    <ReaderSection id="source-summary" title="저장된 요약과 원제">
      {data.koreanSummary ? <><span className={styles.tag}>저장된 한국어 요약</span><p className={styles.prose}>{data.koreanSummary}</p></> : <p className={styles.empty}>표시할 한국어 요약이 없습니다. 제목의 단어만으로 투자 주제나 방향을 추정하지 않습니다.</p>}
