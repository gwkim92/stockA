...
export function ResearchProvenance({ research: rawResearch }: { research: unknown }) {
  ...
  return <details className={styles.disclosure} open={mode === 'fallback' || needsCorrection}>
    <summary>
      {needsCorrection ? '원천 검토 · 보완 필요' : mode === 'fallback' ? '대체 보고서 · 실제 AI 생성 결과 아님' : mode === 'ai' ? 'AI 생성 보고서 · 내용 검토 미기록' : '생성 경로 미확인'}
    </summary>
