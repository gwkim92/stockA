...
export function CompanyWorkspace({ data }: { data: CompanyData }) {
  ...
  const chapters: [string, string][] = [['company-case', '핵심 논리'], ['company-prices', '가격 기록'], ['company-analysis', data.fundKind ? '구성과 비용' : '재무·가치'], ['company-evidence', '뉴스 근거'], ['company-context', '시장 문맥']];
  return <div data-testid="company-workspace" className={styles.page}>
    <header className={styles.header}><div><span className={styles.eyebrow}>{data.fundKind ? 'FUND RESEARCH' : 'COMPANY RESEARCH'} · {data.market}</span><h1>{data.symbol} <span>{data.name}</span></h1><p>분석 기준 {data.asOf ?? '미확인'}</p></div><ReaderLink href={`/stocks/${encodeURIComponent(data.symbol)}/details`}>전문 분석 전체 보기 →</ReaderLink></header>
    ...
    <div className={styles.workbench}><div className={styles.main}><ResearchTabs tabs={chapters}>
      ...
      <Section id="company-analysis" title={data.fundKind ? '보유 구성과 비용' : '재무와 추정 가치'}>
        {data.fundKind ? <>
          ...
          <div className={styles.tableScroll} tabIndex={0} role="region" aria-label="재무 지표 비교 표"><table><caption>저장된 재무 지표 · 행별 기간과 단위를 확인하세요</caption><thead><tr><th scope="col">지표</th><th scope="col">값</th><th scope="col">단위</th><th scope="col">기간 종료일</th><th scope="col">자료 상태</th></tr></thead><tbody>{rows(data.financial.metrics)?.map((metric, i) => <tr key={i}><th scope="row">{text(metric.label, text(metric.metric_code))}</th><td>{metric.metric_unit === 'ratio' ? percentage(metric.metric_value) : decimal(metric.metric_value)}</td><td>{metric.metric_unit === 'ratio' ? '%' : text(metric.metric_unit, '미확인')}</td><td>{shortDate(metric.period_end, '미확인')}</td><td>{metric.metric_status === 'computed' ? '계산됨' : koCode(text(metric.metric_status))}</td></tr>)}</tbody></table></div>
      ...
      <Section id="company-evidence" title="직접 뉴스와 시장 배경 근거">
        {[['기업 직접 연결', data.events], ['상위 시장·테마 영향', data.macro]].map(([label, items]) => <div key={label as string}><h3>{label as string}</h3>{(items as CompanyData['events'])?.map((event, i) => <article className={styles.news} key={i}><span>{shortDate(event.event_at, '관측일 미기록')} · {koCode(text(event.impact_direction))}</span><h4>{text(event.korean_title, text(event.title))}</h4>{text(event.korean_summary, '') && <p>{text(event.korean_summary)}</p>}<div className={styles.actions}><ReaderLink href={route('ai-evidence', event.ai_evidence_id ?? event.event_id)}>근거 해석 열기 →</ReaderLink>{route('source-documents', event.source_document_id) && <SourcePeek documentId={text(event.source_document_id)} />}</div></article>)}{!(items as CompanyData['events'])?.length && <p className={styles.empty}>{items === null ? '근거 목록 미제공' : '반환된 근거가 없습니다.'}</p>}</div>)}
      </Section>
