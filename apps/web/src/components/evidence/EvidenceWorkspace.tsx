...
export function EvidenceWorkspace({ data }: { data: Interpretation }) {
  ...
  return <ReaderFrame title={data.title} eyebrow="EVIDENCE REVIEW · 해석과 원천 대조" subtitle={`${data.reviewLabel} · 사건 기록 ${shortDate(data.eventAt, '미기록')}`}
    ...
    aside={<>
      ...
      <section className={styles.card}><h2>원천과 연결 대상</h2><ReaderLink href={data.sourceHref}>원천 문서 열기</ReaderLink>{data.sourceMismatch && <p className={styles.warning}>문서 식별자와 연결 경로가 일치하지 않아 원천 링크를 보류했습니다.</p>}{!data.sourceHref && !data.sourceMismatch && <p>직접 원천 문서 연결 미제공</p>}<ReaderLink href={route('stocks', data.symbol)}>연결 기업 리서치 →</ReaderLink><ReaderLink href={data.thesisHref}>연결 투자 논리 →</ReaderLink><ReaderLink href={data.recommendationHref}>연결 추천 판단서 →</ReaderLink><p className={styles.caption}>명시적으로 연결된 경로만 표시합니다. 시장 문맥의 첫 종목·추천을 이 근거의 판단으로 대체하지 않습니다.</p></section>
    </>}>
    <ReaderSection id="evidence-interpretation" title={cluster ? '왜 함께 묶인 기록인가' : '무엇을 해석한 기록인가'}>
      ...
      <ReaderFacts items={[["분류된 테마", text(data.classification.theme_name, text(data.classification.theme_key))], ["기록된 방향", koCode(text(data.classification.impact_direction))], ["영향 점수", decimal(data.classification.impact_score)]]} />
