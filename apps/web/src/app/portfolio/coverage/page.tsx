...
export default async function PortfolioCoveragePage({ searchParams }: { searchParams: Promise<{ date?: string | string[] }> }) {
  ...
  return <ReviewFrame kind="portfolio" result={result}>{report && view && <>
    <ReviewMetrics items={[
      { label: "측정 포지션 평가액", value: money(view.valuation.marketValue, view.currency), detail: `${view.valuation.measuredPositionCount}/${view.rows.length}개 · 현금 제외` },
      { label: "측정 포지션 평가손익", value: money(view.valuation.unrealizedPnl, view.currency), detail: `${view.excluded}개 평가자료 미확인·제외` },
