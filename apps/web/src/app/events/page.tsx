...
export default async function EventsPage() {
  ...
  const eventCommandCards = [
    ...
    },
    {
      index: "03",
      label: "AI 연결",
      title: linkedCount > 0 ? `${linkedCount.toLocaleString("ko-KR")}건 연결` : "AI 구조화 전",
      ...
      body:
        linkedCount > 0
          ? "AI 근거가 연결된 뉴스는 구조화 상세에서 원문, 번역, 판단 근거, 추천 연결 여부를 추적한다."
          : "수집은 됐지만 아직 AI 구조화 근거가 연결되지 않았다. 추천 입력으로 쓰기 전 단계다.",
      href: "/ai-evidence",
      cta: "AI 근거 보기",
    ...
    },
    {
      ...
      title:
        blockedOrSuppressedCount > 0
          ...
          : "차단 없음",
      metric: `저신호 ${data.summary.suppressed_low_signal_candidate_count.toLocaleString("ko-KR")}건 · AI 미연결 ${data.summary.unreviewed_event_count.toLocaleString("ko-KR")}건`,
      body:
        blockedOrSuppressedCount > 0
    ...
    <div className="pageStack news-ledger-page">
      <section className="page-hero reveal" aria-labelledby="news-ledger-title">
        ...
        <p className="page-lede">
          이 화면은 투자 결론이나 주문을 내리는 곳이 아니다. 수집 목록, 1차 분류, AI 근거 연결,
      ...
      <section className="events-command-panel reveal delay-1" aria-labelledby="events-command-title">
        <div className="events-command-lead">
          ...
          <p>
            ...
            뉴스는 수집 목록, 분류, AI 구조화, 검증 통과를 거친 뒤에만 추천 근거 후보가 된다.
          </p>
        </div>
