...
export default async function HomePage() {
  ...
  const recommendationItems: readonly DecisionListItem[] = recommendationRows
    ...
    .slice(0, 5).map((row, index) => {
      ...
      return {
        ...
        description: text(boundary.reason, text(evidence.summary, "투자 논리·촉매·반대 근거·무효화 조건을 상세에서 확인하세요.")),
        status,
  ...
  return <div className={styles.page} data-testid="research-home">
    ...
    <nav className={styles.journey} aria-label="투자 판단 경로"><Link href="/market-map">시장 읽기 →</Link><Link href="/stocks">기업 분석 →</Link><Link href="/portfolio/coverage">보유 재검토 →</Link><Link href="/performance/recommendations">판단 성과 →</Link></nav>
