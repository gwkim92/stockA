...
from stockanalysis.ingest.sec.models import SecCompanyFactsSyncResult
from stockanalysis.ingest.sec.financial_periods import select_statement_facts
...
def normalize_companyfacts_payload(payload: dict[str, Any]) -> SecCompanyFactsSyncResult:
    ...
    return select_statement_facts(
        payload, concepts=_CONCEPT_TO_METRIC_CODE,
        instant_metrics=_POINT_IN_TIME_METRIC_CODES, units=_UNIT_BY_METRIC_CODE,
    )
    ...
    *,
    ...
    run_id = _create_pipeline_run(
        ...
        config_json={
            ...
            "period_policy": result.period_policy,
            "selected_facts": [
                {"metric_code": value.metric_code, **(value.source_evidence or {})}
