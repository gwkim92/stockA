...
DEFAULT_PROFILE_SCHEDULES = {
    ...
    "toss-reference-kr-daily": "30 8 * * 1-5",
    "toss-reference-us-daily": "45 8 * * 1-5",
    "toss-candles-kr-daily": "10 16 * * 1-5",
    "toss-candles-us-shadow-daily": "20 18 * * 1-5",
    "toss-priority-microdata-intraday": "40 9,12,15 * * 1-5",
    "toss-live-account-readonly": "55 8,12,16 * * 1-5",
    "cross-asset-daily": "50 18 * * 1-5",
    ...
    *,
    ...
    if runtime_root is not None:
        ...
        report["batch_runtime"] = {
            ...
            "status": "protected" if observed and shared_applied and all(item["resource_limits_applied"] for item in observed) else "unprotected",
        }
    _assert_secret_free(report)
    return report
