...
from stockanalysis.operations.portfolio_review_feedback_calibration import (
    DEFAULT_LIMIT as DEFAULT_PORTFOLIO_REVIEW_CALIBRATION_LIMIT,
    DEFAULT_LOOKBACK_DAYS as DEFAULT_PORTFOLIO_REVIEW_CALIBRATION_LOOKBACK_DAYS,
    DEFAULT_MAX_CONTRADICTION_RATE as DEFAULT_PORTFOLIO_REVIEW_CALIBRATION_MAX_CONTRADICTION_RATE,
    DEFAULT_MIN_FEEDBACK_RUNS as DEFAULT_PORTFOLIO_REVIEW_CALIBRATION_MIN_FEEDBACK_RUNS,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    recommendation_outcome_due_action_router = subparsers.add_parser(
        "recommendation-outcome-due-action-router-run",
        help="Route due recommendation outcome windows to safe calibration without changing weights or orders.",
    )
    recommendation_outcome_due_action_router.add_argument("--env-file")
    recommendation_outcome_due_action_router.add_argument("--as-of-date", required=True)
    recommendation_outcome_due_action_router.add_argument(
        "--horizon-day",
        type=int,
        action="append",
        default=[],
    ...
    recommendation_outcome_due_action_router.add_argument("--market-code")
    recommendation_outcome_due_action_router.add_argument("--strategy-name")
    recommendation_outcome_due_action_router.add_argument("--horizon-type")
    recommendation_outcome_due_action_router.add_argument("--universe-version")
    recommendation_outcome_due_action_router.add_argument(
        "--outcome-version",
        default=DEFAULT_RECOMMENDATION_OUTCOME_VERSION,
    )
    recommendation_outcome_due_action_router.add_argument("--min-sample-size", type=int, default=DEFAULT_MIN_SAMPLE_SIZE)
    recommendation_outcome_due_action_router.add_argument(
        "--min-professional-coverage-rate",
        type=float,
        default=DEFAULT_MIN_PROFESSIONAL_COVERAGE_RATE,
    )
    recommendation_outcome_due_action_router.add_argument("--limit", type=int)
    recommendation_outcome_due_action_router.add_argument(
        "--example-limit",
        type=int,
        default=DEFAULT_OUTCOME_CALIBRATION_EXAMPLE_LIMIT,
    )
    recommendation_outcome_due_action_router.add_argument("--execute", action="store_true")
    recommendation_outcome_due_action_router.add_argument("--dry-run", action="store_true")
    recommendation_outcome_due_action_router.add_argument("--output")
    recommendation_outcome_due_action_router.add_argument("--repo-root", default=str(DEFAULT_REPO_ROOT))
    recommendation_outcome_due_action_router.set_defaults(
        handler=_handle_recommendation_outcome_due_action_router_run
    )
    ...
    portfolio_review_feedback_calibration = subparsers.add_parser(
        "portfolio-review-feedback-calibration-run",
        help="Aggregate portfolio review feedback artifacts before any manual weight-pilot readiness.",
    )
    portfolio_review_feedback_calibration.add_argument("--env-file")
    portfolio_review_feedback_calibration.add_argument("--portfolio-name", default=DEFAULT_PORTFOLIO_NAME)
    portfolio_review_feedback_calibration.add_argument("--as-of-date", required=True)
    portfolio_review_feedback_calibration.add_argument(
        "--lookback-days",
        type=int,
        default=DEFAULT_PORTFOLIO_REVIEW_CALIBRATION_LOOKBACK_DAYS,
    )
    portfolio_review_feedback_calibration.add_argument(
        "--limit",
        type=int,
        default=DEFAULT_PORTFOLIO_REVIEW_CALIBRATION_LIMIT,
    )
    portfolio_review_feedback_calibration.add_argument(
        "--min-feedback-runs",
        type=int,
        default=DEFAULT_PORTFOLIO_REVIEW_CALIBRATION_MIN_FEEDBACK_RUNS,
    )
    portfolio_review_feedback_calibration.add_argument(
        "--min-mature-decisions",
        type=int,
        default=DEFAULT_PORTFOLIO_REVIEW_CALIBRATION_MIN_MATURE_DECISIONS,
    )
    portfolio_review_feedback_calibration.add_argument(
        "--max-contradiction-rate",
        type=float,
        default=DEFAULT_PORTFOLIO_REVIEW_CALIBRATION_MAX_CONTRADICTION_RATE,
    )
    portfolio_review_feedback_calibration.add_argument("--execute", action="store_true")
    portfolio_review_feedback_calibration.add_argument("--dry-run", action="store_true")
    ...
    fund_nav_premium_discount_ssga_spdr_import = subparsers.add_parser(
        "fund-nav-premium-discount-ssga-spdr-import-run",
        help="Download official State Street SPDR product data and import source-backed fund NAV/premium-discount metrics.",
    )
    fund_nav_premium_discount_ssga_spdr_import.add_argument("--env-file")
    fund_nav_premium_discount_ssga_spdr_import.add_argument("--symbol", default="SPY")
    fund_nav_premium_discount_ssga_spdr_import.add_argument("--source-html")
    fund_nav_premium_discount_ssga_spdr_import.add_argument("--source-url", default=DEFAULT_SSGA_SPDR_SPY_PRODUCT_URL)
    fund_nav_premium_discount_ssga_spdr_import.add_argument("--raw-html-output")
    fund_nav_premium_discount_ssga_spdr_import.add_argument("--source-name", default=DEFAULT_SSGA_FUND_METRIC_SOURCE_NAME)
    fund_nav_premium_discount_ssga_spdr_import.add_argument("--execute", action="store_true")
    fund_nav_premium_discount_ssga_spdr_import.add_argument("--dry-run", action="store_true")
    fund_nav_premium_discount_ssga_spdr_import.add_argument("--output")
    ...
    segment_footnote_evidence = subparsers.add_parser(
        ...
        help="Create SEC segment and footnote evidence for SOTP without changing recommendation weights.",
    )
    segment_footnote_evidence.add_argument("--env-file")
    segment_footnote_evidence.add_argument("--as-of-date", required=True)
    segment_footnote_evidence.add_argument("--statement-scope", choices=VALUATION_STATEMENT_SCOPES, default="annual")
    segment_footnote_evidence.add_argument("--execute", action="store_true")
    segment_footnote_evidence.add_argument("--dry-run", action="store_true")
    segment_footnote_evidence.add_argument("--output")
    segment_footnote_evidence.add_argument("--repo-root", default=str(DEFAULT_REPO_ROOT))
    segment_footnote_evidence.set_defaults(handler=_handle_segment_footnote_evidence_run)
...
def _handle_portfolio_review_feedback_calibration_run(args: argparse.Namespace, *, stdout: TextIO) -> int:
    if bool(args.execute) and bool(args.dry_run):
        raise ValueError("--execute and --dry-run cannot be used together.")
    env_mapping = _load_optional_env_mapping(args.env_file, repo_root=args.repo_root)
    as_of_date = date.fromisoformat(args.as_of_date)
    with _temporary_environ(env_mapping):
        report = run_portfolio_review_feedback_calibration(
            ...
            lookback_days=args.lookback_days,
            limit=args.limit,
            min_feedback_runs=args.min_feedback_runs,
            min_mature_decisions=args.min_mature_decisions,
...
def _handle_segment_footnote_evidence_run(args: argparse.Namespace, *, stdout: TextIO) -> int:
    if bool(args.execute) and bool(args.dry_run):
        raise ValueError("--execute and --dry-run cannot be used together.")
    env_mapping = _load_optional_env_mapping(args.env_file, repo_root=args.repo_root)
    as_of_date = date.fromisoformat(args.as_of_date)
    ...
    if args.output:
        output_path = resolve_output_path(
            args.output,
            label="segment footnote evidence output",
            repo_root=args.repo_root,
            require_repo_outside=True,
        )
        write_json_report(report, output_path=output_path, stdout=stdout)
...
def _handle_recommendation_outcome_due_action_router_run(
    args: argparse.Namespace,
    *,
    stdout: TextIO,
) -> int:
    ...
    if args.output:
        output_path = resolve_output_path(
            args.output,
            label="recommendation outcome due action router output",
            repo_root=args.repo_root,
            require_repo_outside=True,
        )
        write_json_report(report, output_path=output_path, stdout=stdout)
