...
class FrontendLiveAdapterTests(unittest.TestCase):
    ...
    def test_live_data_health_response_matches_frontend_contract_shape(self) -> None:
        ...
        toss_market_data = payload["data"]["tossinvest_market_data"]
        self.assertEqual(toss_market_data["sync"]["status"], "missing")
        self.assertEqual(toss_market_data["sync"]["provider"], "tossinvest")
        self.assertTrue(toss_market_data["sync"]["attention_required"])
        self.assertFalse(toss_market_data["sync"]["broker_submit_allowed"])
        self.assertFalse(toss_market_data["sync"]["submitted_to_broker"])
        self.assertEqual(toss_market_data["sync"]["order_boundary"], "read_only_no_order")
        self.assertEqual(toss_market_data["provider_comparison"]["status"], "missing")
        self.assertTrue(toss_market_data["provider_comparison"]["canonical_promotion_blocked"])
    ...
    def test_live_stock_sql_uses_canonical_tables(self) -> None:
        ...
        self.assertIn("toss_provider_latest as", detail_sql)
        self.assertIn("toss_provider_comparison_latest as", detail_sql)
        self.assertIn("market.tossinvest_daily_candle_snapshot", detail_sql)
        self.assertIn("market.tossinvest_provider_comparison_snapshot", detail_sql)
        self.assertIn("bar.provider", detail_sql)
        self.assertIn("bar.source_run_id", detail_sql)
        self.assertIn("'provider', (select provider from latest_price)", detail_sql)
        self.assertIn("'source_run_id', (select source_run_id from latest_price)", detail_sql)
    ...
    def test_live_recommendation_detail_sql_links_score_components_to_event_or_ai_evidence(self) -> None:
        ...
        self.assertIn("'broker-reality-' || lower(recommendation.primary_symbol)", sql)
        ...
        self.assertIn("'broker_reality_context'", sql)
        self.assertIn("'broker_component_name', component.component_name", sql)
        self.assertIn("'broker_note'", sql)
