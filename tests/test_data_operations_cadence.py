...
class DataOperationsCadenceTests(unittest.TestCase):
    def test_cadence_report_exposes_intraday_daily_weekly_monthly_jobs(self) -> None:
        ...
        toss_candles_job = next(job for job in report["jobs"] if job["job_id"] == "toss-candles-us-shadow-daily")
        self.assertEqual(toss_candles_job["pipeline_name"], "tossinvest_market_data_sync")
        self.assertEqual(toss_candles_job["domain"], "market")
        self.assertIn("tossinvest-market-data-sync-run", toss_candles_job["command_template"])
        self.assertIn("--outputsize 30", toss_candles_job["command_template"])
        self.assertIn("--max-symbols-per-run 10", toss_candles_job["command_template"])
        self.assertIn("tossinvest", toss_candles_job["required_env_groups"])
        self.assertEqual(toss_candles_job["data_health_dataset"], "market.tossinvest_daily_candle_snapshot")
        toss_microdata_job = next(job for job in report["jobs"] if job["job_id"] == "toss-priority-microdata-intraday")
        self.assertEqual(toss_microdata_job["pipeline_name"], "tossinvest_market_data_sync")
        self.assertEqual(toss_microdata_job["cadence"], "intraday")
        self.assertIn("--max-symbols-per-run 10", toss_microdata_job["command_template"])
        self.assertEqual(toss_microdata_job["data_health_dataset"], "market.tossinvest_market_microdata_snapshot")
        toss_account_job = next(job for job in report["jobs"] if job["job_id"] == "toss-live-account-readonly")
        self.assertEqual(toss_account_job["pipeline_name"], "tossinvest_readonly_sync")
        self.assertEqual(toss_account_job["domain"], "portfolio")
        ...
        news_eval_job = next(job for job in report["jobs"] if job["job_id"] == "news-ai-eval-intraday")
        self.assertEqual(news_eval_job["pipeline_name"], "news_ai_extraction_quality")
        self.assertEqual(news_eval_job["domain"], "ai")
        self.assertEqual(news_eval_job["cadence"], "intraday")
        self.assertIn("news-ai-eval-run", news_eval_job["command_template"])
        self.assertIn("--provider fixture", news_eval_job["command_template"])
        self.assertNotIn("openai_or_llm_provider", news_eval_job["required_env_groups"])
        self.assertEqual(news_eval_job["data_health_dataset"], "ai.eval_run")
        ...
        self.assertIn("segment-footnote-evidence-run", segment_job["command_template"])
        self.assertEqual(segment_job["data_health_dataset"], "research.segment_footnote_evidence")
        ...
        review_feedback_action_router_job = next(
            job for job in report["jobs"] if job["job_id"] == "portfolio-review-feedback-action-router-daily"
        )
        self.assertEqual(
            review_feedback_action_router_job["pipeline_name"],
            "portfolio_review_feedback_action_router",
        )
        self.assertEqual(review_feedback_action_router_job["domain"], "portfolio")
        self.assertEqual(review_feedback_action_router_job["cadence"], "daily")
        self.assertIn(
            "portfolio-review-feedback-action-router-run",
            review_feedback_action_router_job["command_template"],
        )
