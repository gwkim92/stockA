...
class DataOperationsCliTests(unittest.TestCase):
    ...
    def test_news_rss_ai_extract_run_uses_env_llm_provider_when_provider_omitted(self) -> None:
        with tempfile.TemporaryDirectory() as repo_root, tempfile.TemporaryDirectory() as outside_root:
            env_file = Path(outside_root) / "data-operations.env"
            env_file.write_text(
                'STOCKANALYSIS_PSQL_COMMAND="docker exec psql"\n'
                'STOCKANALYSIS_LLM_PROVIDER="agents_sdk_openai"\n',
                encoding="utf-8",
            )
            stdout = io.StringIO()
            ...
            self.assertEqual(exit_code, 0)
            call_kwargs = runner_mock.call_args.kwargs
            self.assertEqual(call_kwargs["provider"], "agents_sdk_openai")
            self.assertEqual(call_kwargs["model_name"], "codex-cli-default")
    ...
    def test_news_rss_translation_run_uses_env_llm_provider_when_provider_omitted(self) -> None:
        with tempfile.TemporaryDirectory() as repo_root, tempfile.TemporaryDirectory() as outside_root:
            env_file = Path(outside_root) / "data-operations.env"
            env_file.write_text(
                'STOCKANALYSIS_PSQL_COMMAND="docker exec psql"\n'
    ...
    def test_stale_direct_impact_cleanup_run_command_passes_env_execute_and_writes_output(self) -> None:
        with tempfile.TemporaryDirectory() as repo_root, tempfile.TemporaryDirectory() as outside_root:
            env_file = Path(outside_root) / "data-operations.env"
            output_path = Path(outside_root) / "stale-direct-impact-cleanup.json"
            env_file.write_text('STOCKANALYSIS_PSQL_COMMAND="docker exec psql"\n', encoding="utf-8")
            stdout = io.StringIO()

            with patch("stockanalysis.operations.cli.run_stale_direct_impact_cleanup") as runner_mock:
                runner_mock.return_value = {
                    "report_name": "cycle_ai_stale_direct_impact_cleanup",
                    "status": "completed",
                    "candidate_count": 1,
                    "removed_count": 1,
                }
            ...
            self.assertEqual(stdout.getvalue().strip(), str(output_path.resolve()))
            payload = json.loads(output_path.read_text(encoding="utf-8"))
            self.assertEqual(payload["report_name"], "cycle_ai_stale_direct_impact_cleanup")
            call_kwargs = runner_mock.call_args.kwargs
            self.assertEqual(call_kwargs["config"].psql_command, "docker exec psql")
    ...
    def test_portfolio_risk_budget_guardrail_run_command_passes_env_and_writes_output(self) -> None:
        with tempfile.TemporaryDirectory() as repo_root, tempfile.TemporaryDirectory() as outside_root:
            env_file = Path(outside_root) / "data-operations.env"
            output_path = Path(outside_root) / "portfolio-risk-budget-guardrail.json"
            env_file.write_text('STOCKANALYSIS_PSQL_COMMAND="docker exec psql"\n', encoding="utf-8")
            stdout = io.StringIO()

            with patch("stockanalysis.operations.cli.run_portfolio_risk_budget_guardrail") as runner_mock:
                runner_mock.return_value = {
                    "report_name": "portfolio_risk_budget_guardrail",
                    "status": "completed",
            ...
            self.assertEqual(stdout.getvalue().strip(), str(output_path.resolve()))
            payload = json.loads(output_path.read_text(encoding="utf-8"))
            self.assertEqual(payload["report_name"], "portfolio_risk_budget_guardrail")
            call_kwargs = runner_mock.call_args.kwargs
            self.assertEqual(call_kwargs["config"].psql_command, "docker exec psql")
    ...
    def test_portfolio_review_decision_outcome_feedback_run_command_passes_env_and_writes_output(self) -> None:
        with tempfile.TemporaryDirectory() as repo_root, tempfile.TemporaryDirectory() as outside_root:
            env_file = Path(outside_root) / "data-operations.env"
            output_path = Path(outside_root) / "portfolio-review-decision-feedback.json"
            env_file.write_text('STOCKANALYSIS_PSQL_COMMAND="docker exec psql"\n', encoding="utf-8")
            stdout = io.StringIO()

            with patch("stockanalysis.operations.cli.run_portfolio_review_decision_feedback") as runner_mock:
                runner_mock.return_value = {
            ...
            self.assertEqual(stdout.getvalue().strip(), str(output_path.resolve()))
            payload = json.loads(output_path.read_text(encoding="utf-8"))
            self.assertEqual(payload["report_name"], "portfolio_review_decision_outcome_feedback")
            call_kwargs = runner_mock.call_args.kwargs
            self.assertEqual(call_kwargs["config"].psql_command, "docker exec psql")
            self.assertEqual(call_kwargs["portfolio_name"], "Long Term Paper")
            self.assertEqual(call_kwargs["as_of_date"], date(2026, 5, 27))
            self.assertEqual(call_kwargs["history_eval_run_id"], 52)
            self.assertEqual(call_kwargs["min_horizon_days"], 30)
            self.assertTrue(call_kwargs["execute"])

    def test_portfolio_review_feedback_calibration_run_command_passes_env_and_writes_output(self) -> None:
        with tempfile.TemporaryDirectory() as repo_root, tempfile.TemporaryDirectory() as outside_root:
            env_file = Path(outside_root) / "data-operations.env"
            output_path = Path(outside_root) / "portfolio-review-feedback-calibration.json"
            env_file.write_text('STOCKANALYSIS_PSQL_COMMAND="docker exec psql"\n', encoding="utf-8")
            stdout = io.StringIO()

            with patch("stockanalysis.operations.cli.run_portfolio_review_feedback_calibration") as runner_mock:
                runner_mock.return_value = {
            ...
            self.assertEqual(stdout.getvalue().strip(), str(output_path.resolve()))
            payload = json.loads(output_path.read_text(encoding="utf-8"))
            self.assertEqual(payload["report_name"], "portfolio_review_feedback_calibration")
            call_kwargs = runner_mock.call_args.kwargs
            self.assertEqual(call_kwargs["config"].psql_command, "docker exec psql")
            self.assertEqual(call_kwargs["portfolio_name"], "Long Term Paper")
            self.assertEqual(call_kwargs["as_of_date"], date(2026, 5, 27))
            self.assertEqual(call_kwargs["lookback_days"], 365)
            self.assertEqual(call_kwargs["limit"], 25)
            self.assertEqual(call_kwargs["min_feedback_runs"], 3)
            self.assertEqual(call_kwargs["min_mature_decisions"], 10)
            self.assertEqual(call_kwargs["max_contradiction_rate"], 0.15)
            self.assertTrue(call_kwargs["execute"])

    def test_portfolio_review_feedback_cadence_run_command_passes_env_and_writes_output(self) -> None:
        with tempfile.TemporaryDirectory() as repo_root, tempfile.TemporaryDirectory() as outside_root:
            env_file = Path(outside_root) / "data-operations.env"
            output_path = Path(outside_root) / "portfolio-review-feedback-cadence.json"
            env_file.write_text('STOCKANALYSIS_PSQL_COMMAND="docker exec psql"\n', encoding="utf-8")
            stdout = io.StringIO()

            with patch("stockanalysis.operations.cli.run_portfolio_review_feedback_cadence") as runner_mock:
                runner_mock.return_value = {
                    "report_name": "portfolio_review_feedback_cadence",
                    "status": "completed",
                    "eval_run_id": 8401,
            ...
            self.assertEqual(stdout.getvalue().strip(), str(output_path.resolve()))
            payload = json.loads(output_path.read_text(encoding="utf-8"))
            self.assertEqual(payload["report_name"], "portfolio_review_feedback_cadence")
            call_kwargs = runner_mock.call_args.kwargs
            self.assertEqual(call_kwargs["config"].psql_command, "docker exec psql")
            self.assertEqual(call_kwargs["portfolio_name"], "Long Term Paper")
            self.assertEqual(call_kwargs["as_of_date"], date(2026, 5, 27))
    ...
    def test_fund_expense_ratio_ssga_spdr_import_run_command_passes_source_and_output(self) -> None:
        with tempfile.TemporaryDirectory() as repo_root, tempfile.TemporaryDirectory() as outside_root:
            ...
            with patch("stockanalysis.operations.cli.run_ssga_spdr_fund_expense_ratio_import") as runner_mock:
                runner_mock.return_value = {
                    "report_name": "fund_expense_ratio_ssga_spdr_import",
                    "status": "completed",
                    "metric_value": "0.000945",
                }
                exit_code = main(
                    [
            ...
            self.assertEqual(stdout.getvalue().strip(), str(output_path.resolve()))
            payload = json.loads(output_path.read_text(encoding="utf-8"))
            self.assertEqual(payload["report_name"], "fund_expense_ratio_ssga_spdr_import")
            call_kwargs = runner_mock.call_args.kwargs
            self.assertEqual(call_kwargs["config"].psql_command, "docker exec psql")
    ...
    def test_fund_nav_premium_discount_ssga_spdr_import_run_command_passes_source_and_output(self) -> None:
        with tempfile.TemporaryDirectory() as repo_root, tempfile.TemporaryDirectory() as outside_root:
            ...
            with patch("stockanalysis.operations.cli.run_ssga_spdr_fund_nav_premium_discount_import") as runner_mock:
                runner_mock.return_value = {
                    "report_name": "fund_nav_premium_discount_ssga_spdr_import",
                    "status": "completed",
                    "metric_count": 4,
                }
                exit_code = main(
            ...
            self.assertEqual(stdout.getvalue().strip(), str(output_path.resolve()))
            payload = json.loads(output_path.read_text(encoding="utf-8"))
            self.assertEqual(payload["report_name"], "fund_nav_premium_discount_ssga_spdr_import")
            call_kwargs = runner_mock.call_args.kwargs
            self.assertEqual(call_kwargs["config"].psql_command, "docker exec psql")
            self.assertEqual(call_kwargs["source_html"], source_html.resolve())
            self.assertEqual(call_kwargs["raw_html_output"], raw_output.resolve())
    ...
    def test_recommendation_outcome_due_action_router_command_passes_env_and_filters(self) -> None:
        with tempfile.TemporaryDirectory() as repo_root, tempfile.TemporaryDirectory() as outside_root:
            env_file = Path(outside_root) / "data-operations.env"
            output_path = Path(outside_root) / "recommendation-outcome-due-action-router.json"
            env_file.write_text('STOCKANALYSIS_PSQL_COMMAND="docker exec psql"\n', encoding="utf-8")
            stdout = io.StringIO()

            with patch("stockanalysis.operations.cli.run_recommendation_outcome_due_action_router") as runner_mock:
                runner_mock.return_value = {
                    "report_name": "recommendation_outcome_due_action_router",
                    "status": "completed",
                    "eval_run_id": 8701,
                    "action": {"action_status": "no_op_wait_until_next_due_date"},
                }
                exit_code = main(
                    [
            ...
            self.assertEqual(stdout.getvalue().strip(), str(output_path.resolve()))
            payload = json.loads(output_path.read_text(encoding="utf-8"))
            self.assertEqual(payload["report_name"], "recommendation_outcome_due_action_router")
            call_kwargs = runner_mock.call_args.kwargs
            self.assertEqual(call_kwargs["config"].psql_command, "docker exec psql")
            self.assertEqual(call_kwargs["as_of_date"], date(2026, 5, 27))
            self.assertEqual(call_kwargs["horizon_days"], (30,))
    ...
    def test_portfolio_attribution_run_command_passes_env_and_writes_output(self) -> None:
        with tempfile.TemporaryDirectory() as repo_root, tempfile.TemporaryDirectory() as outside_root:
            env_file = Path(outside_root) / "data-operations.env"
            output_path = Path(outside_root) / "portfolio-attribution.json"
            env_file.write_text('STOCKANALYSIS_PSQL_COMMAND="docker exec psql"\n', encoding="utf-8")
            stdout = io.StringIO()

            with patch("stockanalysis.operations.cli.run_portfolio_attribution_monthly") as runner_mock:
                runner_mock.return_value = {
            ...
            self.assertEqual(stdout.getvalue().strip(), str(output_path.resolve()))
            payload = json.loads(output_path.read_text(encoding="utf-8"))
            self.assertEqual(payload["report_name"], "portfolio_attribution_monthly")
            call_kwargs = runner_mock.call_args.kwargs
            self.assertEqual(call_kwargs["config"].psql_command, "docker exec psql")
            self.assertEqual(call_kwargs["portfolio_name"], "Long Term Paper")
            self.assertEqual(call_kwargs["as_of_date"], date(2026, 5, 27))
            self.assertEqual(call_kwargs["methodology"], "position_weighted_alpha_v1")
            self.assertTrue(call_kwargs["execute"])
    ...
    def test_financial_forecast_inputs_run_command_passes_env_and_guardrails(self) -> None:
        with tempfile.TemporaryDirectory() as repo_root, tempfile.TemporaryDirectory() as outside_root:
            env_file = Path(outside_root) / "data-operations.env"
            env_file.write_text('STOCKANALYSIS_PSQL_COMMAND="docker exec psql"\n', encoding="utf-8")
            stdout = io.StringIO()

            with patch("stockanalysis.operations.cli.run_financial_forecast_inputs") as runner_mock:
                runner_mock.return_value = {
            ...
            self.assertEqual(exit_code, 0)
            payload = json.loads(stdout.getvalue())
            self.assertEqual(payload["report_name"], "financial_forecast_inputs")
            self.assertFalse(payload["recommendation_scoring_mutated"])
            call_kwargs = runner_mock.call_args.kwargs
            self.assertEqual(call_kwargs["config"].psql_command, "docker exec psql")
            self.assertEqual(call_kwargs["as_of_date"], date(2026, 5, 25))
    ...
    def test_sum_of_parts_valuation_run_command_passes_env_and_guardrails(self) -> None:
        with tempfile.TemporaryDirectory() as repo_root, tempfile.TemporaryDirectory() as outside_root:
            ...
            with patch("stockanalysis.operations.cli.run_sum_of_parts_valuation") as runner_mock:
                runner_mock.return_value = {
            ...
            self.assertEqual(exit_code, 0)
            payload = json.loads(stdout.getvalue())
            self.assertEqual(payload["report_name"], "sum_of_parts_valuation")
            self.assertFalse(payload["recommendation_scoring_mutated"])
            call_kwargs = runner_mock.call_args.kwargs
            self.assertEqual(call_kwargs["config"].psql_command, "docker exec psql")
            self.assertEqual(call_kwargs["as_of_date"], date(2026, 5, 25))
    ...
    def test_segment_footnote_evidence_run_command_passes_env_and_guardrails(self) -> None:
        with tempfile.TemporaryDirectory() as repo_root, tempfile.TemporaryDirectory() as outside_root:
            env_file = Path(outside_root) / "data-operations.env"
            env_file.write_text('STOCKANALYSIS_PSQL_COMMAND="docker exec psql"\n', encoding="utf-8")
            stdout = io.StringIO()

            with patch("stockanalysis.operations.cli.run_segment_footnote_evidence") as runner_mock:
                runner_mock.return_value = {
            ...
            self.assertEqual(exit_code, 0)
            payload = json.loads(stdout.getvalue())
            self.assertEqual(payload["report_name"], "segment_footnote_evidence")
            self.assertFalse(payload["recommendation_scoring_mutated"])
            call_kwargs = runner_mock.call_args.kwargs
            self.assertEqual(call_kwargs["config"].psql_command, "docker exec psql")
            self.assertEqual(call_kwargs["as_of_date"], date(2026, 5, 25))
    ...
    def test_reported_segment_footnote_parser_run_command_passes_env_and_guardrails(self) -> None:
        with tempfile.TemporaryDirectory() as repo_root, tempfile.TemporaryDirectory() as outside_root:
            env_file = Path(outside_root) / "data-operations.env"
            env_file.write_text('STOCKANALYSIS_PSQL_COMMAND="docker exec psql"\n', encoding="utf-8")
            stdout = io.StringIO()

            with patch("stockanalysis.operations.cli.run_reported_segment_footnote_parser") as runner_mock:
                runner_mock.return_value = {
            ...
            self.assertEqual(exit_code, 0)
            payload = json.loads(stdout.getvalue())
            self.assertEqual(payload["report_name"], "reported_segment_footnote_parser")
            self.assertFalse(payload["recommendation_scoring_mutated"])
            call_kwargs = runner_mock.call_args.kwargs
            self.assertEqual(call_kwargs["config"].psql_command, "docker exec psql")
            self.assertEqual(call_kwargs["as_of_date"], date(2026, 5, 25))
