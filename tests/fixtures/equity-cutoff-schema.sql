...
create table market.financial_metric_normalized (instrument_id bigint, metric_code text, metric_value numeric, metric_unit text, metric_status text, rationale text, statement_scope text, period_end date, as_of_date date, source_run_id bigint);
create table ref.peer_group (peer_group_id bigint, group_code text, name text);
