...
class RecommendationFundamentalComponentsTests(unittest.TestCase):
    ...
    def test_upsert_sql_adds_zero_weight_components_without_mutating_totals(self) -> None:
        ...
        self.assertIn("when 'free_cash_flow_to_net_income'", sql)
        self.assertIn("when 'accrual_ratio'", sql)
        self.assertIn("when metric_code = 'liabilities_to_assets'", sql)
        self.assertIn("where metric_code in ('leverage_ratio', 'liabilities_to_assets')", sql)
        self.assertIn("earnings quality/accruals", sql)
